...
> **Superseded 2026-09-09: every row is named, whatever is connected.** `showsProductAttribution` is gone and nothing replaces it. **The same day, the second gate went too** — the live list is one block per product at every count, one included ([`expanded-panel-v2.md`](expanded-panel-v2.md) §4.3 rule 01) — so *who says the name* is no longer a question either: a closed live row is named by the heading above it, and a retired row and an open row draw their own chip. Both are true at one connected product and at two, and neither changes when a second product connects.
>
> What the paragraph above got wrong is that it read the badge as a comparison. It is a label. A row names the product it belongs to for the same reason it names its Project: that is what the row *is*, and a reader arriving at one row is not holding the rest of the list in their head to tell it apart from. The spoken form has always said this — a retired row names its product whether or not a badge is drawn ([`dual-agent-design.md`](dual-agent-design.md) §4) — and the drawing now agrees with it.
>
> **The cost the paragraph names is real and is accepted.** The lit ink at rest on every row is `10 pt` of text inside a `16` pt chip, the second-brightest thing in a row and by a long way the smallest; the row's brightness ranking is unchanged (the next paragraph), and the caption line is `16` with a chip and without one, so no row moves. What is bought is that the panel stops changing what a row says about itself when a *second, unrelated* product connects.
...
| A row's breadcrumb below the seam ([`expanded-panel-v2.md`](expanded-panel-v2.md)) | All four presentations reach it | The badge, in place of the prefix — ~~**and it stays**, because the queue is not grouped: its whole reading is an age, and the ages are one descent a heading would restart at every block~~ **on the flat queue**; the grouped queue (2026-09-14, [`expanded-panel-v2.md`](expanded-panel-v2.md) §4.7) moves it to the block's heading, as the live list does |
...
| `ProductAttributionStyle` | **Retired**, with `MonitorStore.productAttribution` and its defaults read. ~~`showsProductAttribution` stays — it is what gates the badge.~~ **`showsProductAttribution` is retired too, and so is `groupsSessionsByProduct`** (2026-09-09): nothing gates the badge and nothing gates the blocks, so the presence rule has no reader left anywhere. The stored `productAttribution` key is left in place unread, which is what "ignored rather than migrated" means in practice |
...
| `NotchPalette.aggregateInk(_:isConnected:)` | Unchanged. `badgeInk(_:)` returns the same pair, so the two cannot drift; it takes no `isConnected`. ~~because a badge is drawn only while more than one product is connected and the resting grey is a state it cannot be in~~ **The reason changed and the signature did not** (2026-09-09): a badge is now drawn with nothing connected at all — the queue holds a row for five hours after its product closes — and it is still right that it takes none. The resting grey means *nothing is connected*, which is a claim about the surface; a badge names the product that published this row, which is a fact about the row, and a row cannot stop having been published |
...
- [x] ~~A badge appears on a row's caption only while more than one product is connected, and disappears when the second disconnects — not when it merely runs out of threads.~~ **Restated 2026-09-09**: every row is named at one connected product exactly as at two — the live row by its block's heading, the retired row and the open row by their own chip — and nothing about either is different at the moment a second product connects (`everyLiveRowIsUnderAHeadingAtEveryProductCount`, `aOneProductListIsDrawnAsOneBlock`).
