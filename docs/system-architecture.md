...
| Hook product Provider | `HookProductProvider`, `ProductPresenceReporting`, `RunningApplicationPresence`, `ThreadAdmitting`, `ThreadAdmission` | The Provider a hook-based product gets by default: managed setup, the helper and its socket, the shared reducer, and one row per Turn — with the two facts the hooks cannot supply handed in: whether the product is open (`RunningApplicationPresence` for a desktop product, which is also the reading Codex's service takes — the pid and the presence beside it from one look, since the pid binds its hook evidence; a CLI supplies its own, and `ClaudeCodeSessionListing` is one) and which Threads the product vouches for (`ThreadAdmission`: every observed Thread, an exact list read at an instant, or unknown — and only a completed Turn on a Thread absent from a list that postdates it is retired, never a live one). What the row can say is the vocabulary's: start and end alone is Tier 0, wait events Tier 1, an `answering` encoding Tier 2. No quota or interruption evidence: each is a capability a product composes beside it. The row's line is the Turn's closing words, then this Turn's text from the preview store, which the Provider prunes to the Turns it holds on every refresh — empty for a vocabulary with no message event, Antigravity's transcript reads otherwise. **Read state it does compose**, for a product that hands in `TerminalReadEvidence` — the same gate, settling window, dismissal rule and 1-second re-check the two shipping actors run, and `nextRefreshDeadline()` reports that re-check and nothing else. A product that hands in none never builds a gate entry and reports no deadline at all. The two shipping products still run their own actors; this is the destination they migrate towards, built first so that a third product can be wired before either migrates ([`tiered-support.md`](technical-explorations/multi-product-provider-architecture/tiered-support.md) §5.4, §8 P4). Held by `HookProductConformanceTests`, the Tier 0 and Tier 1 fixtures of §7 | [`HookProductProvider.swift`](../Notchline/Notchline/HookProductProvider.swift) |
