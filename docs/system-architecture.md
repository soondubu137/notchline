...
The process table is sampled at most once every five seconds, independently of the number of refreshes; known owners are validated on refresh and again for navigation. Changed execution ownership wakes the Provider, ordinary repeated provenance does not. Disconnect clears ownership and deadlines. No subprocess polling or additional overlay animation is introduced. See [technical design](tech-design.md#codex-local-cli-boundary-2026-09-15) for mode/home checks and [product support](product-support.md) for native acceptance limits.

Native validation rejected a Hook-derived current-view binding: `/new` delays SessionStart until the next prompt. CLI rows are therefore excluded from automatic read judgement, and CLI navigation raises only the verified host. This also avoids terminal read polling for CLI completed rows.

Release-optimised source benchmark on macOS 26.6.2 / CLI 0.154.0, measured with cumulative process CPU (`getrusage`): local TUI classification **0.233 ms/call** over 1,000 calls; full inventory **0.538 ms/scan** over 100 scans; PID/creation-time liveness **0.00764 ms/call** over 10,000 calls. At one scan per five seconds, inventory alone averages approximately **0.011% of one core**. This is the incremental adapter cost, not a whole-overlay steady-state measurement; no UI rendering code changed.
