...
- **这道闸门问的是「这一轮」，不是「这条 thread 现在」。** 此前它在组行时直接查映射，而 Desktop 是在审查者被改动的那一刻就重写 `heartbeat-thread-permissions-by-id` 的，正在跑的那一轮却仍然沿用它开始时的审查者、继续一条条问人。实测 2026-08-24：thread `01a03241` 以 `user` 起轮（22:32:13），22:37:35 被切成 `auto_review`，此后 22:40:07、22:40:28、22:41:10、22:42:10、22:42:28 五次审批对话框全部由人亲手答复（用户报上来的那条 `rm -rf` 就在其中），而该轮 rollout 的 `turn_context.approvals_reviewer`（真正的权威）全程仍是 `user`；`01a03130` 在五小时前是同一形状。于是答案由 `TurnApprovalRoutingPin` 在**首次看见该 Turn 时取一次并钉住**，直到该 Turn 从 reducer 里消失为止；下一轮重新问映射。**双向对称**：中途切成 `user` 的 thread，这一轮也仍然算自动审查。钉住只能钉住第一眼看到的东西——若本进程对某个 Turn 的第一次刷新就落在映射翻转之后，钉住的就是翻转后的答案；该窗口只有一次刷新宽（`UserPromptSubmit` 会立刻唤醒一次），彻底关掉它要改读 rollout 的 `turn_context`，成本见下表脚注。
