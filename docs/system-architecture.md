...
| The global chord | `KeyChord`, `PanelChordHold`, `PanelChordHolder` | Register one system-wide hotkey and say what happened. `RegisterEventHotKey` is the ordinary path and was chosen because it **answers**: it returns a status, where a key monitor that another application has out-competed simply never fires, which is the failure `answer-in-notch.md` §9.3 exists to rule out. The holder lives on `OverlayPanelController` — a registration is a live system resource rather than a value the store can hold — and the store publishes only the outcome, which the settings row draws. The preference stores a **position on the keyboard**, not a character; the drawn form comes from the current layout through `UCKeyTranslate`, so the same chord is drawn in the letters of whichever layout is in front | [`PanelChord.swift`](../Notchline/Notchline/PanelChord.swift) |
...
### A published delta is lost in a burst; a published total is not (2026-09-06)

The keyboard scrolls the open row's body (`answer-in-notch.md` §6.2), and the offset it moves must stay in the body's own `@State` for the reason the section above gives — publishing it would put a whole-panel re-render behind every wheel event. So the store publishes what the *keyboard* has asked for and the body applies it, which is one publish per keystroke and costs what a click already costs.

Published as **one press's delta**, four `↓` in quick succession moved the body **one line**. `onChange` compares the published value once per render pass, and four presses inside one pass are one change — three deltas were never seen. A serial number beside the delta does not help: it makes the change detectable, not recoverable. Published as a **running total**, the body applies the difference between the total and the total it last saw, and a burst of any length arrives intact: measured on Release, `+54` lines under the fold falling to `+50`.

The general rule, and it applies to anything else this store hands the panel: **a published value is sampled, not delivered.** Anything whose meaning is "what changed" must be published as a position, or it is only as reliable as the render loop's timing.
...
So latching activates this app for as long as a row is open and activates the previous application again when the row closes ([ADR 0020](adr/0020-the-panel-takes-the-keyboard-by-activating.md)). **Since the chord landed (2026-09-06) the latch has two doors** — a click on a mark, and `⌥Space` — so `MonitorStore.isLatched` is `openRowID != nil || isKeyboardEngaged` rather than the first alone: with nothing waiting the chord latches a panel with no open row on it, and a panel that held no keys there would be one `⎋` could not close. The panel controller merges both publishers and reads `isLatched` back off the store after the main-queue hop, because `@Published` emits from `willSet` and the value in flight is the old one. Verified on Release: with a row open `lsappinfo front` names Notchline, and after the second `⎋` it names the application that was in front before the chord. Two flags were in the way and each produced a symptom that looked like the same bug: `becomesKeyOnlyIfNeeded`, which makes AppKit refuse `makeKeyAndOrderFront` outright, and an `NSTextView` built with a `nil` text container, which takes the caret, receives `keyDown` and inserts nothing.
