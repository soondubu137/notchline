...
flowchart LR
    ...
    subgraph appServerBoundary ["公开 App Server 只读边界"]
        ...
        transportGuard["stdio JSON-RPC 握手与 NDJSON buffer"]
    ...
    appServerProcess <-->|"stdio"| transportGuard
    transportGuard <-->|"JSON-RPC response"| appServerClient
    requestGuard -->|"保护 request 生命周期"| appServerClient
    appServerClient <-->|"五个只读方法与结果"| liveService
...
| 类型 | 图中能力 | 约束 |
| --- | --- | --- |
| 官方公开 | Hooks lifecycle、App Server 协议与六个只读方法、`codex://threads/{threadId}` | 作为主集成契约使用 |
...
sequenceDiagram
    ...
    loop Hook active 每 1 秒或未连接每 5 秒
        ...
        alt 当前 Desktop PID 中已有启动后 Hook 观察
            ...
            opt Hook dirty 新 thread 或 30 秒到期
                service->>appServer: schedule background thread/list
                appServer-->>service: cached list replacement on full success
            end
...
flowchart LR
    ...
    store --> notchServer
    desktopServer -.->|"没有公开的跨进程当前态查询"| notchServer
    notchServer -->|"thread/list 成功即完成本轮快照\nactive turns 或确认空集合"| notch
```

状态判断分为传输与快照两个阶段：`initialize` 或连接失败、App Server 没有响应时才是 Disconnected；握手已经开始但 `thread/list` 尚未完成时保持 Connecting；`thread/list` 成功返回即确认本轮快照，存在活动 Turn 时发布对应会话状态，没有活动 Turn 时以 Ready 空集合发布并聚合为 Idle。`thread/loaded/list` 只保留为传输超时后的轻量探活，不再决定业务 availability 或启动成员集合。

独立 App Server 与 Desktop 仍不共享进程内事件流，因此启动快照只使用该 App Server 当前返回的公开 Thread 状态；启动后的 Hook 负责增量变化。完整共享运行时仍属于 [`technical-explorations/shared-app-server/README.md`](technical-explorations/shared-app-server/README.md) 中的后续探索。在任何拓扑下都不得用启动 cutoff 之前的事件补齐当前状态。
...
- `PermissionRequest` 只触发新鲜度校正，不直接制造 `Approval needed`；只有同一 Turn 的新鲜 `waitingOnApproval` 才能确认该状态。
- 产品只关心 Turn 是否仍在进行：实时 `Stop` 以及 App Server 的 `completed`、`failed`、`interrupted` 都直接成为 Completed，不再发起 `thread/read` 区分结束原因。
- App Server thread-level `activeFlags` 只能校正已由精确 Turn 身份绑定的当前 Turn，不能创建 Turn、不能把 Completed 改回活动态，也不能复活 `retiredTurnIDs` 中的历史 Turn。
...
flowchart LR
    ...
    pending --> buffer["NewlineDelimitedMessageBuffer"]
    buffer --> response["matched response"]
...
| 公开协议边界 | `CodexAppServerClient` | 子进程、stdio JSON-RPC、握手、超时、探活与传输重建 | [`CodexAppServerClient.swift`](../CodexInNotch/CodexInNotch/CodexAppServerClient.swift) |
| 纯解析 | `CodexSnapshotParser` | 解析 Thread、Turn、activeFlags、四态信号、标题、预览、额度并排序 | [`LiveCodexMonitorService.swift`](../CodexInNotch/CodexInNotch/LiveCodexMonitorService.swift) |
