...
**Two counts hang off the mark, computed in the same merge as the status.** Besides fixing a `MonitorStatus` per connected product, `MonitorAggregation.marks` counts that product's own rows (`PresenceMark.sessionCount`, drawn as the dot column right of the matrix) and its unfinished subagents (`PresenceMark.subagents`, drawn as the badge before the timer, its ground saying whether any of them is waiting on a person). Both look only at that product's own rows — the same filter as the status — so Codex's Turns are never counted into Claude Code's mark. The collapsed badge pair reads that result directly (`MonitorStore.compactSubagentBadges`) rather than re-summing, so the two ends of one bar cannot give two answers about one list. The session count dots (`SessionCountDots`) are part of the mark rather than an addition to it: `PanelMetrics.markWidth` counts the matrix and the `5.66` column to its right as one mark, and `marksWidth(_:areProductMarks:)` does so only for product marks, since the resting grey mark has no column. The column is **reserved in width and packed in drawing in the expanded header**, which is sized from a baseline rather than from its contents, so a column opening inside it cannot widen anything and the room has to be there already: `marksWidth` cannot be told the session count, so that panel's mark group is one width at every count; `StatusReadout` frames it into that width aligned to the leading edge, a product with no rows draws no column, and the width given up is drawn back over by the status name. **Both collapsed forms reserve nothing** and are composed from `drawnMarksWidth(markCount:sessionColumnCount:)` instead, so each is exactly as wide as what it draws. The notched bar is pinned to the cut-out, so its leading wing can only take width leftwards, and a column opening moves the leading edge and the matrices ahead of it while the marks between it and the cut-out stand still. The pill is centred, so it takes half the column on each edge and everything after the new dot glides by half of it — the movement its own reservation used to buy out, at the price of holding that room permanently against its trailing edge, where it read as a margin four times the leading one (`figma-design.md` §6.4). Its trailing wing hugs the reading in the same way (`drawnTrailingReadingWidth`), **and so, since the pill gave up its `00:00:00` slot, does the pill's trailing slot** — one expression for both collapsed forms, which is why `elapsedLayoutRevision` now moves the panel rather than only re-measuring inside it, on either geometry. The pill is centred, so it takes half of each widening on each edge; the notched bar is pinned to the cut-out and takes all of it on the trailing side. The reading is the one thing left that either of them reserved: the pill's resting width is the marks and the longest name the working set can say, and nothing else ([`figma-design.md`](figma-design.md) §6.4). The column is exactly as tall as the matrix (`2 × 5.84 + 4.93 = 16.6`), so it is unaffected by menu-bar height and draws identically at every step.
...
| --- | --- | --- | --- |
| UI state | `MonitorStore` | Pull complete snapshots, merge refresh triggers, publish UI state, compute the top-level summary, and remove rows on user intent (right-click on one row, in any state, recorded per product in `dismissedSessionIDsByAgent`, forgotten only once that product is visible and no longer lists that Turn, `tech-design.md` §17) | [`MonitorStore.swift`](../Notchline/Notchline/MonitorStore.swift) |
