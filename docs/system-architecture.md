...
| Updating the app | `AppUpdater` | Own Sparkle's `SPUStandardUpdaterController`. Start it only outside tests and only when the build carries `SUPublicEDKey`; checking daily (Release) and on About → Check for Updates, with scheduled alerts shown behind the front app. It draws nothing in the overlay and reads nothing from the store ([ADR 0022](adr/0022-update-through-sparkle-signed-with-our-own-certificate.md)) | [`AppUpdater.swift`](../Notchline/Notchline/AppUpdater.swift) |
