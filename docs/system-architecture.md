...
| 设置窗口 | `AppSettingsView`、`ProductSettingsCopy`、`MacOSWindowColor` | macOS 26 单面板设置：分组卡片自绘，控件全用原生；`Color / macOS Window` 两模式 token（见 `figma-design.md` §8）。产品行说的那几句话是一个值（`ProductSettingsCopy`）而不是四个 view 上的计算属性——那一行下方的失败报告是本窗口里唯一为报告失败而存在的东西，值可以被断言，`body` 不能（CR-029）。窗口**怎么出现**归 `SettingsWindowPresenter`：先读有焦点的那块屏（在排窗与激活之前，见 `PRD.md` §11），再激活本应用并把窗口排到最前；只有窗口不在那块屏上时才移动它，落点算法是纯函数 `SettingsWindowPlacement`，可断言。`⌘,` 走的是 SwiftUI 自己的菜单项、本应用看不见，因此另有一条路：`SettingsWindowTracker` 把 scene 的窗口交给 presenter，presenter 观察它的 `isVisible` 由假转真——那一刻窗口已排出但尚未取得 key，正是还能问出「刚才焦点在哪」的最后一刻 | [`SettingsWindow.swift`](../Notchline/Notchline/SettingsWindow.swift) |
