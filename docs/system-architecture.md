...
8. **「正在忙」不是丢弃请求的理由**：任何不能并发执行自己的操作都用 [`SingleFlightGate`](../CodexInNotch/CodexInNotch/SingleFlightGate.swift)，而不是一个 `isXXXInFlight` 布尔。布尔看起来没问题，但它只在「一定会有别的东西再问一次」时才安全，而这个前提在边沿触发的信号上不成立。gate 用单调 revision 记住运行期间到达的请求，跑完自动补一次；失败的运行不算覆盖，请求留到退避结束后继续；需要看到自己那次请求结果的调用方（比如 Recheck）可以等自己的 revision。开关这类「最后一次说了算」的意图也走同一个 gate，只是把 dirty 位换成 desired state——不重放中间状态，只收敛到最后一次。
...
10. **承诺不落盘的东西就不要落盘**：prompt 与回答正文只经 socket 进入内存，事件文件只携带身份与生命周期。推论是隐私开关也不落盘——写盘的开关会失败、会乱序，那正是 CR-012 的 fail-open。凡是「用户以为已经关掉」的东西，都必须在调用返回前就已生效。
