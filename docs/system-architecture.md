...
**The expanded header is the one form still drawn the V1 way**, and the one form that still reserves: `PanelMetrics.marksWidth(_:areProductMarks:)` gives every mark its session-dot column at every count, `StatusReadout` packs them into that width from the leading edge, a product with no rows draws no column, and the width given up is drawn back over by the status name. It is sized from a baseline rather than from its contents, so a column opening inside it cannot widen anything and the room has to be there already. [`expanded-header-v2.md`](expanded-header-v2.md) folds it to match the bar; until then the two leading groups differ, deliberately and visibly, on hover.
