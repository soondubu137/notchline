...
| Hook transport, wired once | `HookLifecycleSource` | The three every hook-based product needs, connected the one way they connect: the `ManagedHooksSetup` that writes the registration and the helper, the `HookEventRepository` its events land in, and the `AgentHookListener` whose delivery closure is that reducer's `deliver` and whose socket is the one the helper names. `ClaudeCodeMonitorService` and `HookProductProvider` compose it instead of wiring the three themselves; a test still injects any of them. Codex keeps its own wiring for now, because its setup is `CodexHookRegistrar` | [`HookLifecycleSource.swift`](../Notchline/Notchline/HookLifecycleSource.swift) |
