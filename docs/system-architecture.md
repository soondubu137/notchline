...
This document follows the current Swift implementation, from a product's boundary signals entering the app through row-level status convergence, set filtering, top-level summary and per-surface navigation. The main chain stays:
...
```mermaid
flowchart LR
    subgraph codexSources ["Codex Desktop, local CLI and data sources"]
        desktopHooks["Codex official Hooks lifecycle"]
    ...
    subgraph hookBoundary ["Hook boundary"]
        hookRegistrar["CodexHookRegistrar actor, ten frozen definitions"]
    ...
    hookRegistrar -->|"appends missing definitions at the tail, never rewritten"| hooksConfig
...
- **Which rows the gate is asked about is one rule for every product, not one per Provider.** `TerminalUnreadRowFilter` walks a refresh's rows for all three: a row the user removed is still reported and judged by nobody (CR-Fable-003, CR-Fable-004); nothing is judged, and the gate is emptied, unless a finished row the user has not removed is listed (CR-Fable-041); a row nothing can speak for (`ReadGateVerdict.cannotBeAsked`) is shown and never enters the gate (CR-Fable-036); the gate is given the thread's status rather than the row's; and it is retained to exactly the rows judged, **plus the read verdict of every Turn the reducer still holds**. Hiding is final for a Turn, not for a row. A Turn can draw no row for one refresh and still be held; a Trae Turn does this whenever its companion connection drops. Until 2026-09-16 that one refresh dropped the verdict, so the row came back unread. A hidden entry books no re-check, so keeping it costs nothing. A closed product or a stopped observation keeps it too; only the reducer letting the Turn go ends it. What a product supplies is only the verdict — Desktop's unread set for Codex, the five routes for Claude Code, the terminal's verdict for a CLI product. Until 2026-09-12 each of the three Providers carried its own copy of this loop.
...
| Exact navigation (Codex) | `CodexDesktopNavigator` | Pre-flight the target and open the same Thread through the official deep link | [`CodexDesktopNavigator.swift`](../Notchline/Notchline/Products/Codex/CodexDesktopNavigator.swift) |
| Navigation dispatch | `AgentNavigationRouter` | Hand a whole row to its own product's navigator; a product with no registered navigator fails under its own name rather than being handed to the first in the table | [`AgentNavigation.swift`](../Notchline/Notchline/AgentNavigation.swift) |
...
| Updating the app | `AppUpdater`, `UpdateFlow`, `UpdateStatus` | Sparkle's `SPUUpdater` with Notchline as its `SPUUserDriver`, so an update is drawn on the notch and in Settings → Updates, never in a Sparkle window ([`updates-on-the-notch.md`](updates-on-the-notch.md)). Started only outside tests and only when the build carries `SUPublicEDKey`. `AppUpdater` turns callbacks into events for the pure `UpdateFlow`, which decides every state, and publishes `UpdateStatus`. That is `@Observable`, and progress reaches it at most 4 times a second. From the store it reads only whether About is on screen and which rows a relaunch would cut off ([ADR 0022](adr/0022-update-through-sparkle-signed-with-our-own-certificate.md)) | [`AppUpdater.swift`](../Notchline/Notchline/AppUpdater.swift), [`UpdateFlow.swift`](../Notchline/Notchline/UpdateFlow.swift) |
...
**Since 2026-09-15 the field can grow** ([`answer-in-notch.md`](answer-in-notch.md) §7.1). That makes its height a layout fact, so it has to be published, but only when a line is gained or lost. `AnswerFieldBox` hosts the text view in a scroller with no bars and draws the ground and rail in AppKit, so scrolling the field redraws that box and not the panel. The store measures each edit's draft with a TextKit 1 layout in a container four lines tall (`PanelMetrics.answerFieldLineCount`), cached against the text and wrap width. That is `0.02` ms for 20 characters, `0.1` ms for 200 and `3.1` ms for a 10,000-character paste, averaged over 2,000 calls in an optimised standalone build of the same code; it is not a whole-app Release measurement.
...
`TraeProvider` composes one `ProductMonitoringRuntime` with `TraeSource` as lifecycle, session reading and row content. `TraeBridgeTransport` owns directory discovery, same-user socket validation, bounded newline framing and ordered `TraeEvidenceBoundary` processing on one serial queue. Only typed evidence/progress enters `MonitoringRepository`; no native schema reaches `MonitorSnapshot` consumers. There is no additional Turn reducer, answer channel or quota scheduler. `TraeReadEvidence` supplies per-candidate read judgements to the existing `TerminalUnreadRowFilter`. After the gate, the runtime hands every row source the finished Turns whose rows are over, hidden as read or removed by the user (`RowContentSource.releaseEndedRows`). Only `TraeSource` acts on it: the boundary stops admitting those Turns, so a reconnect stops asking the companion for them. It asks for at most 96 Threads, open Turns first ([Trae integration](trae-integration.md)).
...
## Codex CLI composition (2026-09-15)

`LiveCodexMonitorService` remains the only Codex orchestration centre. Production composition supplies `CodexSurfaceLedger`, a provenance boundary over `CodexProcessSource`, and shared host navigation. Hook callbacks capture the native peer and submit through the existing ordered evidence boundary; the ledger never reduces Turn state. It decides one thing besides ownership: a request from a local TUI is submitted **without** its descriptor, so Codex — which waits for the Hook rather than asking beside it — keeps its own prompt in the terminal, and that row is reading-only ([tech design](tech-design.md#codex-local-cli-boundary-2026-09-15)). `MonitoringSignal.turnInterrupted` ends only an already observed exact Turn. `retainOwnedThreads` retires execution ownership without advancing the product-wide epoch.

The process table is sampled opportunistically inside an existing refresh, with a five-second cache and no inventory deadline; known owners are validated on refresh and again for navigation. Changed execution ownership wakes the Provider, ordinary repeated provenance does not. Disconnect clears ownership and deadlines. No subprocess polling or additional overlay animation is introduced. See [technical design](tech-design.md#codex-local-cli-boundary-2026-09-15) for mode/home checks and [product support](product-support.md) for native acceptance limits.

Native validation rejected a Hook-derived current-view binding: `/new` delays SessionStart until the next prompt. CLI navigation therefore raises only the verified host. CLI-only completed rows do receive `TerminalReadEvidence`: a later keystroke, paste or terminal focus report, paired with the foreground host and an available screen. It identifies the TUI, so it may retire every ended Turn that TUI still owns; it never names the displayed Thread. Desktop/CLI shared ownership has no unique read authority. The Provider samples only ended, undismissed rows that need read evidence, through the existing read gate and deadline. Ghostty focus reports and Apple Terminal's lack of them were verified on 2026-09-15; pointer motion is not a Codex gesture.

Release-optimised source benchmark on macOS 26.6.2 / CLI 0.154.0, measured with cumulative process CPU (`getrusage`): local TUI classification **0.233 ms/call** over 1,000 calls; full inventory **0.538 ms/scan** over 100 scans; PID/creation-time liveness **0.00764 ms/call** over 10,000 calls. At a hypothetical one scan per five seconds, inventory alone averages approximately **0.011% of one core**; this is not a scheduled cadence, since inventory itself books no wake-up. This is the incremental adapter cost, not a whole-overlay steady-state measurement; no UI rendering code changed.
