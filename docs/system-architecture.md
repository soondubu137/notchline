...
- **Codex asks a person in two shapes, and only one of them is a wait (measured 2026-09-07, CLI `0.153.4`).** It ships two question handlers, and **the model decides which a Turn gets** — not a setting a user could be asked about. On an isolated `CODEX_HOME` with the hooks trusted by their own `currentHash`, `gpt-6-astra` (this machine's default) registered only `request_user_input_async` in Default mode, while `gpt-5.6-sol` registered the blocking `request_user_input`; and Plan mode gave the blocking one on `gpt-6-astra` too. So both reach the same user on the same day, by route rather than by configuration.
  ...
  - **`request_user_input_async` does not.** Same event, same id discipline, but its `PostToolUse` arrived **51 ms** after the open carrying `{"accepted":true}` whatever the person does, and the Turn ran another tool and reached its own `Stop` afterwards; any answer arrives later as a **new user message**, which is a new Turn. Its arguments are `{"questions":[{"title": …}]}` — a different shape from the blocking tool's, with no field an answer could be written into. It is therefore an ordinary tool call to this app, and reading it as a wait would announce Input needed for 51 ms and withdraw it. What the person is owed is already delivered: Codex emits the question as its own assistant message, and it reaches the row inside the Turn's `Stop` as `last_assistant_message`.
