...
| 终端已读边界（Claude Code） | `ControllingTerminalGestureReader` | **对每个会话都问，不是上一条答 unknown 时才问**（远程控制会让同一个会话两边都在）：`sysctl(KERN_PROC_PID)` → 控制终端设备号 → `devname_r` → `stat` 的**访问时间**，即最后一次有东西从这个设备被读走；**不在屏幕上的界面收不到任何东西**，所以它按会话而不是按应用成立；没有控制终端答 `nil`，那样的行不进 gate | [`ControllingTerminalGestures.swift`](../CodexInNotch/CodexInNotch/ControllingTerminalGestures.swift) |
