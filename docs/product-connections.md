...
Codex installation is the union of Desktop and a standalone executable, including `~/.local/bin`, `/opt/homebrew/bin`, `/usr/local/bin` and PATH. Either can satisfy installation discovery. Presence is the union of Desktop and verified local interactive processes; the CLI inventory is refreshed at most every five seconds, with kernel APIs and no command subprocess. A failed inventory cannot claim that no CLI exists. Current live Hook ownership still supplies positive presence.
