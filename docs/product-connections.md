...
Explicit Off stops observation, removes the product's rows and requests removal of the settings owned by Notchline. Because it does all three at once, the switch asks first: nothing is recorded, removed or hidden until the user confirms the dialog ([what the switches do](integration-settings-behaviour.md) §3). Cancelling leaves intent enabled. A removal failure retains Off and reports a retryable operation error. It cannot silently resume monitoring just because the external settings remain. Each product's existing convergence task preserves last-flip-wins ordering. Successful direct setup/removal records intent too; an older operation cannot overwrite a newer requested choice.
...
Existing connection retries and lifecycle evidence expiry remain product-owned. Connection presentation introduces no additional continuous UI animation or Turn reconstruction. Codex CLI presence uses the bounded inventory described below.
...
## Codex CLI discovery (2026-09-15)

Codex installation is the union of Desktop and a standalone executable. One list of directories answers for every CLI this app discovers — `~/.local/bin`, `/opt/homebrew/bin`, `/usr/local/bin`, MacPorts' `/opt/local/bin`, the mise, asdf, Volta and Bun shim directories, then PATH, which a window-server launch inherits from launchd rather than a login shell. `CodexExecutableLocator` searches Desktop's own bundled `codex` first and that same list after it, so the binary that runs the App Server and the install Settings names can differ only where intended, never because two searches looked in different places. The list cannot be complete — nvm-style layouts hide the binary under a version directory — and does not need to be: no row depends on it. Either can satisfy installation discovery. Presence is the union of Desktop and verified local interactive processes; the CLI inventory is sampled during an already-needed refresh and cached for five seconds, with kernel APIs and no command subprocess or inventory wake-up deadline. A TUI with no observed Hook becomes visible on the next refresh from another cause, not within a promised five-second interval. A failed inventory cannot claim that no CLI exists. Current live Hook ownership still supplies positive presence.

One settings entry, helper, activation check and account reader serve both surfaces. Unsupported modes and unverifiable provenance do not acquire rows. Closing one terminal or Desktop retires its own ownership only; stopping observation clears the product-wide state and all deadlines. The same ten-event file was accepted by standalone `0.154.0` and Desktop's bundled `0.154.0-alpha.6.2`; this does not replace normal trust review in each running client.

Connection health is shared; support coverage is not. A connected Codex CLI is L5 for ordinary approvals and synchronous questions, with all decisions left in the terminal. Read removal is conditional on terminal gestures; exact terminal navigation is unsupported. [The CLI declaration](product-support.md#51-local-codex-cli) separates these capabilities from unknown variants and account-read availability.
