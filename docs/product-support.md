...
Trae is pinned to the verified 3.5.91 application fingerprints. Local IDE/V2 persistent root Threads are supported. IDE-hosted SOLO remains outside the declared L5 coverage, although [native boundary tests](technical-explorations/multi-product-provider-architecture/trae-solo-boundaries.md) confirm ordinary local lifecycle and structured-question behaviour. It is distinct from standalone SoloLite; the latter, remote workspaces, Plan/Spec and child activity are excluded. Initial snapshots admit no historical or already-running Turn. Observation loss hides rows without inferring completion; no deletion or read-removal source is supplied. The [implementation and native acceptance record](trae-integration.md) states setup, source limits and verification.
