...
Trae is pinned to the verified 3.5.91 application fingerprints. Local IDE/V2 persistent root Threads are supported; SOLO, remote workspaces, Plan/Spec and child activity are excluded. Initial snapshots admit no historical or already-running Turn. Observation loss hides rows without inferring completion; no deletion or read-removal source is supplied. The [implementation and native acceptance record](trae-integration.md) states setup, source limits and verification.
