...
| Antigravity CLI: approvals and questions | Unsupported: neither a wait nor its request content is observed | Unsupported |
...
| Feature | Codex Desktop | Claude Code (Desktop and CLI) | Antigravity CLI |
| --- | --- | --- | --- |
| **Level** | **L6** for ordinary `PermissionRequest`; L4/L5 coverage for synchronous questions and `request_permissions` | **L6**, for the request forms in §3 | **L3**, for observed CLI Turns; mode limits below |
| Turn lifecycle and elapsed time | Supported | Supported | Supported from the first model invocation to `Stop`; interruption without `Stop` is not independently observed |
| Project name | Desktop's Project assignment, or `Chats`; never inferred from cwd | Working directory name | TUI workspace path; current `-p` payloads have no path and display `Untitled folder` |
| Thread title | Desktop name → Thread preview → current prompt → `Untitled` | Transcript title records; `Untitled` when unavailable | User request read from the transcript; prompt-derived, not native-title synchronisation |
...
| Read removal | Desktop's per-Thread unread state | Desktop records and qualified foreground evidence; direct-terminal gestures | Conditional: type or paste after the Turn ends, with its terminal application in front. Merely returning to the tab or moving the pointer does not clear the row |
| Navigation | Exact Thread deep link after target validation | Raise Desktop or terminal host; select a terminal tab/pane where the host supports identity matching | Raise the process's host; select a matching terminal tab/pane where supported |
...
| Terminal reason | No failure reason from hooks; interruption evidence is read separately | Failure reason and interruption evidence where provided | End observed; reliable failure/cancellation classification is unverified |
...
Antigravity's live progress is event-updated, not token streaming. Its interruption experiment did not establish whether `Ctrl-C` always emits `Stop`; a missing end is never synthesised from silence. The [Antigravity measurement](technical-explorations/multi-product-provider-architecture/antigravity-cli.md) records the modes, delays and conservative exits. These limits are part of its L3 declaration.
