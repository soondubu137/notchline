...
| Read removal | Conditional, from the Thread's own controlling terminal: a keystroke, a paste or a focus report after the Turn ended, with that terminal's application in front. Codex sets `ESC[?1004h` and `ESC[?2004h` and no mouse tracking, so — unlike Claude Code — a pointer crossing the window is never a gesture, and the TUI's own output and a Turn ending move nothing. **Whether coming back to the window is enough depends on the terminal**: measured 2026-09-15, Ghostty delivers focus in and out and retires the row on return, while Apple Terminal delivers neither and waits for a keystroke. The report reaches only the focused surface — a second TUI in another window was untouched across the same switch. A Thread Desktop also runs, and `tmux`/`screen`/`ssh` with no application ancestor, stay out of the gate and leave on dismissal, the next submission or loss of ownership. Desktop unread absence has no authority over a terminal |
