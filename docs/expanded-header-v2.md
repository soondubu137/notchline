...
| Version | 2.1 |
| Date | 2026-09-04 |
| Amended | **A third control stands beside the pair, and does not join it** (2026-09-11): `Privacy Mode`'s peek, held down to lift every cover on the list and released to put them back ([`cover-the-words.md`](cover-the-words.md) §7). It is drawn **only while something on the list is covered**, and it takes its box out of the slack between the counts and the pair rather than out of the trailing side's own reservation — so `8 + mark + gear + 12` is unchanged, **no width in this document moves**, and neither of the two controls a user has learnt the position of moves either. The reservation is what keeps it out of the group: `expandedTrailingSideWidth` is two boxes wide and binds the **resting** form, which is the one form that can never have a covered row on it. |
