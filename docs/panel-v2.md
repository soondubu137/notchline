...
So the three can still be built in the order they were argued: the badge and the band, then the footer, then the field this app already receives and throws away — and **all three are now free of open questions**. The third's first half landed on 2026-09-05: the app keeps the request, the reducer holds it on the wait that asked, and it reaches `MonitorSnapshot`. Nothing draws it yet, which is why §6's two boxes below are still unticked.
