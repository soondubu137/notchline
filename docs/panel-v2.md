...
**And the ground is the app's ink rather than white.** `NotchPalette.brightGround` — `themeInk`'s lit `#DEE8E0` — wherever this panel fills a bright ground: the mark, the subagent badge, and the ground `⏎` sits on inside an open row. §1's *a white ground said a person was wanted* is now *the app's own ink says it*, which costs the argument nothing and gains it the thing the rule was really after: the brightest value on the surface is spent on one state rather than on two, because **white is the pointer's now** and the only filled white left is the mark with the pointer on it.
