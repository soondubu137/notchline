...
**A summary reports a maximum, so everything under it is undrawn — and one state cannot afford that.** Approval never loses; Input loses only to Approval, and the mark still says a person is wanted; a Running Turn that loses asks for nobody and ends by itself. `Completed` both loses and waits, and it is in the list at all only while it is unread (§7), so it clears when somebody reads it and never on its own. The collapsed state therefore reports it in a second channel: **a product's session-dot column breathes while that product's list holds a finished, unread Turn its own mark is not drawing** — no new mark, no number and no width, and nothing at all when the mark is already saying it. Treatment and rules in [`dual-agent-design.md`](dual-agent-design.md) §12; the priority table above is unchanged, and a buried Input still stays buried.
