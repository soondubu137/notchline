...
- 通过窗口焦点、路径、标题、时间接近度或 GUI 自动化猜测会话身份、Project、已读状态或导航目标。**例外经产品批准并写在 [ADR 0012](adr/0012-read-state-is-answered-per-product-or-not-at-all.md) 里**，只在 Claude Code 一侧成立，Codex 一侧本条禁令原样有效：某个 Turn 结束**之后**，Claude Desktop 回到前台、或用户往它里面敲了键滚了页，而 Desktop 自己的记录显示它屏幕上的就是那个会话时，该会话算已读。它不是「获得焦点即已读」——每一条都要两件可查的事实合起来才成立，缺一不作数，而且「持有前台」从不单独作数。窗口标题、时间接近度、靠等待本身移除、Accessibility 与 GUI 自动化在任何一侧都仍然禁止；输入只读「上一次按键或滚动离现在多久」，不读键码、字符、位置或窗口。
...
- **Claude Code（Desktop 托管会话）**：四条路径，满足其一即为已读。它们是同一句话的四种形状——**答案在用户面前时，用户做了一个只有人会做的动作**。
  ...
  3. **该 Turn 结束之后，用户往 Claude Desktop 里敲了一个键或滚了一下，而它屏幕上的就是这个会话。** 覆盖「会话本来就在屏幕上、用户压根没离开」——这一种前两条都够不着，因为既没有新的显示，也没有一次回到前台。按键与滚轮投递给持有前台的应用，因此它说明用户是朝着那个窗口做的；鼠标移动不算，点击也不算。手势必须晚于 Turn 的结束时刻，所以把窗口留在前台然后离开座位的用户同样什么都不会被撤掉。
  ...
  用户在 Claude Desktop 归档该会话同样移除。**Turn 结束的那一刻本身不构成任何一条**：那一刻盯着看的人和提交完就走的人做过的最后一件事完全相同，所以行会等到上面四种动作里最先发生的那一个；读完之后一动不动的用户，行会留着（见 ADR 0012）。

- **Claude Code（终端会话）**：**没有任何"已读"概念存在**——Claude Code 自己的会话记录只有 `status` / `waitingFor` / `updatedAt`，不记录焦点。这类会话的终态行按既有方式退出：该会话的下一次提交、会话从官方会话列表消失，或用户手动移除（在该行上右键，或清空整张列表）。右键移除是这类行唯一不牵连整张列表的出口。这是**声明过的能力边界**，与 ADR 0004 对 Claude Code 导航的处理同性质，不得用窗口焦点、终端 tty、固定时间或 Notch 点击伪装成已读（见 [ADR 0012](adr/0012-read-state-is-answered-per-product-or-not-at-all.md)）。
...
3. 活动轮次始终显示；终态轮次在桌面端已读、归档或删除后自动移除——Codex 按未读集合，Claude Code Desktop 托管会话按 4.2 的四条路径（显示时刻晚于该轮次终止时刻；或轮次结束后应用回到前台、或用户往它里面敲键滚页，且它屏幕上的就是该会话；或该会话带着已结束的轮次停在屏幕上之后被别的会话顶下去）。轮次结束的那一刻本身不移除任何一行。终端里的 Claude Code 会话不参与本条，理由见 ADR 0012。
