...
3. 活动轮次始终显示；终态轮次在桌面端已读、归档或删除后自动移除——Codex 按未读集合，Claude Code Desktop 托管会话按 4.2 的四条路径（显示时刻晚于该轮次终止时刻；或轮次结束后应用回到前台；或应用此刻持有前台且屏幕醒着未锁，且两者屏幕上的都是该会话；或该会话带着已结束的轮次停在屏幕上之后被别的会话顶下去）。终端里的 Claude Code 会话不参与本条，理由见 ADR 0012。
