...
**启动前的现状同步按产品成立。** Codex 侧不做——不存在受支持的「此刻在处理什么」查询，这是能力边界而非取舍。Claude Code 侧做：官方会话列表给出存在哪些会话，transcript 给出其中哪些仍在轮次中，因此启动前就在运行的轮次立即显示。重建出的轮次只可能是 Running（等待期间 transcript 不写入，无法区分等待与工作），且任何真实事件都完全覆盖它。
