...
**Approval 排在 Input 之前。** 两者都是产品停下来等人，等的却不是同一种东西：Input 是一个用户什么时候来、什么时候答的问题，Approval 是一个智能体过不去、而用户未必愿意让它过去的决定。矩阵按状态画不同图案之后（advance 与 double knock，[`figma-design.md`](figma-design.md) §4.6），这个顺序还多决定一件事——同时握着两种等待的那条 bar 画哪一个图案。它因此必须是值得打断人的那一个，否则状态名说一句、旁边的矩阵画另一句。~~Input 排第一~~ 已作废；它当初的理由是下面第 2 档那条时序规则，而那条规则说的是同一行的两个读数谁更新，不是两行之间谁更重要。
