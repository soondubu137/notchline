...
叠层挂在 `.statusBar` 层，比菜单栏还高一级，因此系统从不替它让位：全屏播放的影片上面留着刘海，Mission Control 缩小的桌面上面浮着一块面板。**产品规则是「菜单栏在哪儿，面板就在哪儿」**——菜单栏这一条屏幕归系统的时候，这块叠层不属于用户正在看的东西。

- 目标显示器的菜单栏不再绘制时（该显示器上有窗口全屏，或用户把菜单栏设成自动隐藏），面板离屏。
- **Mission Control（以及 App Exposé、Spaces 切换）不隐藏菜单栏**，菜单栏照常画在缩小的桌面之上；它单独成一条，面板同样离屏。
