...
**同一个位置还说另一件事：仍在跑与仍需处理的子智能体总数，画成一到两枚数字 chip。** 需要处理（白色，从不染色）在前、仍在跑（灰色，或单产品连接时染该产品墨色）在后——完整规则见 [`dual-agent-design.md`](dual-agent-design.md) §10。有 chip 要画时它们排在计时前面；所有轮次都结束而子智能体还在跑或在等审批时没有计时可读，那里只剩 chip——这正是「轮次结束了，可这条 Thread 还没有」在收起态的形状，与展开行里同一对 chip 说的是同一件事。两个计数都为零时该位置与引入子智能体之前完全一致，只有计时。它是全表总数，与那一个计时读数为整张列表说话是同一条规矩。无刘海屏幕的胶囊为计时预留的宽度装得下一枚 chip 与短读数的组合，装不下时胶囊自己变宽，而不是把 chip 裁掉。朗读文案用词而不是裸数字（`1 waiting for you, 2 subagents`）。
...
- **行尾只有一个位置，先给计时**：轮次还在计时的时候那里是处理时间；计时停下之后，如果这条 Thread 还有子智能体在跑或在等审批，那里画一到两枚数字 chip——需要处理（白色，从不染色）在前、仍在跑（灰色，展开行内永远不染产品色）在后，规则见 [`dual-agent-design.md`](dual-agent-design.md) §10。两者不并存——正在跑的行已经在说这条 Thread 在工作，再画 chip 只是同一句话说两遍，而**计时停下、位置却不空**恰好是「轮次结束了，可这条 Thread 还没有」的唯一形状。两个计数都为零或读不到子智能体边界时该位置为空，与引入子智能体之前完全一致。
