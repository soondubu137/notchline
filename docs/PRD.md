...
- 带刘海屏幕另有一种收起形态：`Hide the wings`（第 11 节）打开后两侧的翼都不画，收起态即刘海本身。它与「没有任何产品连接时什么都不画」是同一个形态，区别只在前者由用户选定、对任何状态都成立。无刘海屏幕没有这一形态。
...
1. **Display**：两项。其一，选择组件显示在哪个已连接显示器；选择跨启动保留，显示器临时断开时回退到可用屏幕。其二，`Hide the wings`——收起态只留刘海本身，两侧的翼都不画（没有矩阵，也没有计时），面板宽度恒等于遮挡宽度。它**只作用于收起态**：hover 照常落下完整面板，因为刘海是本产品唯一的入口（本节下文：没有菜单栏项，也没有 Dock 图标）。该项跨启动保留，默认关闭；组件此刻在无刘海显示器上时**置灰而不是隐藏**——无刘海的药丸没有可以藏身的缺口，收起它会把它在菜单栏里的位置一起带走并且不再留下任何可 hover 的形状，但偏好本身不清空，换回有刘海屏即恢复。
