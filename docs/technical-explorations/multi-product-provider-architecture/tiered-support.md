...
A terminal Turn from a Tier 0 product retires at the Thread's next submission, when the Thread disappears, or on dismissal. That is the lifecycle `CONTEXT.md` defines for a Turn whose read state cannot be asked about — and it is the **default rather than the ceiling**, which is the distinction this paragraph originally lost. Read state is a capability declared independently of tier (below), the terminal source of it is shared by every CLI product, and a product that can say which process a Thread runs in has already supplied everything that source needs. Antigravity CLI is the case: Tier 0, no wait of any kind, and its finished rows retire on being read ([`antigravity-cli.md`](antigravity-cli.md) §3.1). What stays true at Tier 0 is that nothing is *required* — a product with no such evidence gets the lifecycle above and books no work for a question it cannot ask.
