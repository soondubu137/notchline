...
The prompt is in no payload, so a row is `Untitled`; `tech-design.md` §5's prompt fallback has nothing to fall back to. The transcript the payload names (`brain/<conversationId>/.system_generated/logs/transcript_full.jsonl`) would carry it and is not read at this tier.
