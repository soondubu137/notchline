...
Added **2026-09-12**, and the defect it closes is the one a day of real use produces: a TUI session stays open between turns, so a `Completed` row stood on the notch until the *next* submission however thoroughly its answer had been read. That is the Tier 0 lifecycle `tiered-support.md` §2 describes, and it is the lifecycle of a product whose read state cannot be asked about — which this product's is not. The conversation's process is already known (§3); the terminal that process is attached to is the same source Claude Code's CLI sessions are read by, and `TerminalReadEvidence` is that source composed once for every CLI product rather than twice.
...
| `AntigravityConformanceTests.swift` | Registration in the product's shape beside the user's hooks; the Tier 0 fixtures with the product's payloads; the lost-`Stop`, mid-turn-launch, print-mode and sibling-product cases; presence, admission and the click from one reading; and the five read-state cases of §3.1 | ~500 lines |
...
No Turn state is inferred from silence; no historical hook is replayed; another surface's runtime is declined by its transcript path rather than mistaken for the CLI's; no permission prompt is created to observe one. Read evidence retires a finished Turn and can open none (`AGENTS.md` §6, authority by operation), and a terminal that cannot answer is reported as unanswerable rather than as unread. The product enters Tier 0 and no higher, and the registry ([`non-public-codex-integration-features.md`](../../non-public-codex-integration-features.md)) records what it depends on that no documentation promises.
