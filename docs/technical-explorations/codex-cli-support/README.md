...
~~The target is cumulative **L6 for ordinary command approvals**~~ — **the shipped coverage is L5, and the target was wrong rather than missed** (2026-09-15). The target and the acceptance below both asked whether a decision sent from the notch reaches Codex, which it does; neither asked whether the TUI *also* asks. It does not — Codex waits for the Hook instead of raising a prompt beside it — so a held connection does not add the notch beside the terminal's own question, it replaces it, and the person watching the terminal saw only `Working`. A local TUI's connection is therefore released as soon as its payload is read. Synchronous question reading and waiting stay at L4/L5 as planned, conditional on all lower-level gates passing. Additional request forms need their own samples. No answering control of any kind is offered over Hooks for this surface.
...
| Command approval allowed by Hook | `PreToolUse(Bash)` → `PermissionRequest` → `PostToolUse` → `Stop`; the harmless `printf` executed | Existing decision encoding and request-bound answer channel are viable — **and were withdrawn for this surface anyway** (§1): what the probe answered is that Codex acts on a decision, not that the TUI is still asking while it waits |
...
- Ordinary `PermissionRequest` is read and **not** answered from here (revised 2026-09-15; §1). `PermissionRequest` lacks `tool_use_id` in the sample, so retain association with the preceding call and the existing refusal rules. Resolution comes from native evidence, as it did when an answer was possible.
