...
3. **`pendingInputToolUseID` is also per-agent, but not drawn.** Separating it keeps pairing correct (one slot's `AskUserQuestion` must not be closed by another's `PostToolUse`); not drawing it is because "would a subagent's question actually reach a person" is unmeasured, and a prompt this app cannot vouch for is worse than none. **Measured for Claude Code on 2026-09-12: a subagent cannot ask at all.** It is given no `AskUserQuestion`, neither in its tool list nor among its deferred tools. Seen on a foreground subagent in Claude Desktop 2.1.266 and on three background subagents in a CLI 2.1.270 pty, which emitted no `PreToolUse` or `PermissionRequest` for the tool. So the approval a question's `PermissionRequest` files beside it, which a turn's own row had counted as a second request until `7262a1b`, cannot occur on a subagent's slot. The Codex half is still unmeasured.
