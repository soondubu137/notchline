...
Reviewed on **7 September 2026**. The repository named `open-vibe-island` now calls its application **Open Island**.

Notchline concentrates on identifying the Codex Desktop and Claude Code Turns that still need attention. Open Island covers more coding products and terminal workflows, with more controls for notifications, presentation and continuing work. Neither is a strict superset of the other.

## Scope and evidence

| Project | Snapshot inspected |
| --- | --- |
| Notchline | Local `master`, clean before this documentation change, commit `20ff4f1cb3cae1c97f7f5f135e3cb33d07d463fd` |
| [Open Island][oi-repo] | Default branch, commit [`334c58073ec0ea8a1b34da0c71f969b1affd0959`][oi-snapshot], dated 3 September 2026 |
...
| --- | --- | --- |
| **1. Clearing completed rows using evidence that the Thread was read** | Uses Codex Desktop's unread membership. Claude Code uses Thread-specific Desktop display/read evidence and controlling-terminal gestures. A read completed Turn can leave the live list without dismissing its row manually. | Visibility is based on process/attachment state, activity, completion and presentation thresholds. No equivalent Codex unread-state or Claude Desktop/terminal read-state adapter was found. Suppressing a notification when a terminal is foreground is a separate feature. [N1][n-read] [O1][oi-presence] |
...
## What Open Island provides that Notchline does not provide

| Capability | Open Island | Notchline at the inspected commit |
| --- | --- | --- |
| **1. More coding products and standalone Codex CLI** | Dedicated setup/runtime paths for Cursor, Gemini CLI, Grok Build, Kimi CLI, OpenCode, Pi, Oh My Pi, Qoder, Qwen Code, Factory and CodeBuddy, alongside Claude Code and Codex. Codex CLI is a supported workflow. Observation and answer capabilities vary by product. | Two products: Codex Desktop and Claude Code. A standalone CLI or IDE is not an independent Codex list source; a Codex row must satisfy the Desktop-navigable root Thread contract. Claude Code CLI is already supported. [O9][oi-setup] [N12][n-prd] |
| **2. More precise terminal, pane and IDE return targets** | Dedicated targeting for Ghostty, cmux, Kaku, WezTerm, tmux, Zellij and Warp, as well as Terminal.app/iTerm2. IDE paths include VS Code, Cursor, Windsurf, Trae, Zed and JetBrains launchers, with workspace/app-level fallbacks. | Resolves the actual Claude host through process ancestry. Exact terminal-tab selection is implemented for Terminal.app and iTerm2; other hosts can be raised without those additional pane/workspace adapters. Full-screen Claude hosts are an explicit limitation. [O10][oi-jump] [N13][n-claude-jump] |
...
| **8. Configurable grouping and sorting of the conversation list** | Group by state, coding product or workspace/project; choose attention or last-update sorting. Also offers selectable state indicators, compact-slot content and a configurable completed-to-idle threshold. | Uses a prescribed urgency order and one shared presentation. Display selection, theme/outline/wing and compact-name settings do not provide equivalent grouping/sorting controls. [O18][oi-appearance] [N14][n-settings] |
...
| Codex `request_user_input` / `request_permissions` | Can render request information, but these `PreToolUse` paths have no answer channel here | Rollout parsing can recognise waiting states; no equivalent complete answer route for these tools was found. A manually configured `PreToolUse` allow/deny hook is not that route. |
