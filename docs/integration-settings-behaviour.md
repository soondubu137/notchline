...
- **The Claude Code row** shows `Connected · hooks installed` on `setup.status() == .active` alone — exactly one current-version handler per definition in `settings.json`. With `.active` but availability `.disconnected` it shows `Registered · the hook helper could not be set up` instead.
...
| `Registered · the hook helper could not be set up` (Claude Code) | `.active` + `.disconnected`; for this product that means `prepareTransport()` returned `false`: the helper could not be written or the socket could not be bound, whether the folder is unwritable or another instance of the app already holds it |
