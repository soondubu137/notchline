...
~~With four product rows, Settings scrolls vertically within a 580 pt-wide window, capped at 860 pt or 80 pt below the smallest connected display’s visible height (minimum 420 pt).~~ Since 2026-09-13 Settings is three toolbar panes — Products, Display, Quota — and the window takes the height of the pane in front, so the product switches, the Display controls and Quit are each reachable without scrolling. A pane scrolls only when it is taller than the smallest connected display’s visible height less 140 pt (minimum 320 pt) ([`figma-design.md`](figma-design.md) §8.0).
