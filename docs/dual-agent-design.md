...
**Folding usually closes the panel, and that is the correct answer rather than a side effect.** The chevron rides the footer's last line, so it sits about `14` above the panel's bottom edge — less than the `26` or `56` that folding takes away. A click does not move the pointer, so the bottom edge travels up past a pointer that is standing still, and the pointer is outside the panel the instant the rules go. The panel is held open by hover, so it then collapses on the ordinary `250 ms` dwell: the fold finishes at `200 ms` and the close begins just after it, reading as one continuous movement. Only Codex alone is exempt, and by arithmetic rather than by rule — its control sits `16` to `32` above the edge and folding moves that edge only `12`, so the pointer stays on the panel and the panel stays open.
...
`aFoldedFooterIsTheSameHeightForEveryShape` pins all three forms to `314` folded and `326` / `340` / `370` expanded; `foldingKeepsTodaysTokensInEveryShape` pins the Codex-only exception — no daily-usage line when expanded, and one when folded. `foldingLiftsThePanelsBottomEdgePastTheChevronThatWasClicked` pins the geometry above, and `aResizeThatLeavesThePointerOutsideCollapsesThePanel` the correction it needs.
