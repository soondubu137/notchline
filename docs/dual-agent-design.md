...
The notch-less form **does not widen with the current state**: the timer slot always reserves the widest `00:00:00`, and the whole working set shares the one width computed from the widest timeable status (`Approval`) — `217.85 → 218` for two products and `195.27 → 196` for one ([`figma-design.md`](figma-design.md) §6.8, `PanelMetrics.fixedCompactWidth`). **The notched form does the opposite and reserves nothing at all**: it measures real text, bills the timer for its digits rather than for a slot, and rounds only the whole body up. The trailing wing is therefore `reading + 12 + 8` — `56` at `1:23`, `64` at `10:00`, `84` at `10:00:00` — where the pill holds `65.91 + 12` open at every reading. The table records the composition only.
