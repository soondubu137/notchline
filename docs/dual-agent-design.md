...
无刘海形态的状态名仍然只有一个，保持灰色，取两个产品中最紧急的状态，顺序沿用 `Input needed > Approval needed > Running > Completed`。成对的矩阵已经回答了"是哪一个"，把色相加到状态名上等于把亮度与色相压在同一段短文本上，而亮度是这个界面的注意力通道。带色状态名的方案已绘制备用（Figma §04 B），在实测显示矩阵对未被读取时再启用。
