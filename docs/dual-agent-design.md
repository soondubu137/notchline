...
This used to be the panel's one way to get stuck open. Hover reaches the app through an `NSTrackingArea`, which speaks only when the pointer *moves*; shrink the window away from a still pointer and no exit is ever delivered, and none ever will be, because the pointer is by then outside a window that no longer receives its movement. The panel stayed expanded over nothing until the pointer happened to enter and leave again. `OverlayPanelController` now re-asks the question on every resize (`MonitorStore.panelResized`), which is the same correction the concealment path already made by collapsing on the way out. Only the leaving direction is corrected — a panel that *grows* under a still pointer is left alone, or Escape would be undone by the pointer still resting on the notch.
