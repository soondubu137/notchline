...
| 项目 | 双产品 | 单产品 |
| --- | --- | --- |
| 前导翼 | `12 + 16.6 + 6 + 16.6 + 8 = 59.2` | `12 + 16.6 + 8 = 36.6` |
| 刘海形态总宽 | `307.6 → 308 × 46`（计时 `1:23`） | `285 × 46` |
| 无刘海形态 | `12 + 矩阵 + 12 + 状态名 + 32 + 00:00:00 + 12` | 同左，少一个矩阵与一个间距 |
...
1. **收起态胶囊，只有一个产品已连接：chip 染该产品自己的墨色**——点亮格／熄灭格取 §2 颜色表同一对值（Codex `#6CB4FF` / `#101B26`，Claude Code `#D97757` / `#21120D`）：字取点亮色，底取熄灭色**提一档**后的 `#1F2A35`／`#30211C`（见本节末）。
2. **收起态胶囊，两个产品都已连接：chip 转中性灰**——底 `#242424`（`NotchPalette.restingInk` 的熄灭色同样提一档）、字 `#7C7C80`（`label`／`text/notch-label`，见 `NotchStatusMatrix.swift`）。两种墨色此刻都不准确，谁也不借。
...
**「仍在跑」那枚 chip 的底色统一比熄灭色亮一档。** chip 是一块 `15 × 15` 的实心方块，不是矩阵里那种点：铺到这个面积上，熄灭色在纯黑面板里读成一个洞，而不是一枚放在面板上的标记。所以三种底色（中性灰与两个产品的熄灭色）都在原值上按每个通道 `+0.06` 统一朝白提一档（`MatrixInk.chipFill`，`NotchStatusMatrix.swift`）：色相与中性灰的中性都原样保留，三者与背后的黑保持同一段距离，而且仍然明显是「白底 chip 领先、它在后」这一对里暗的那一端。矩阵自己不用这个值——它要的暗度按 §2 不变。
