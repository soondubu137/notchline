...
**Why `2.8 s`.** Bars is the slowest thing the mark runs, at `2 s` — as the lull it replaced was; sitting below that puts the breath in a different order of movement, so it cannot be taken for a fifth pattern on a mark `2.92` away. It has come through three versions of the swing untouched, and it is the lever still in hand if the movement ever needs more — amplitude was the cheaper one and went first, and the rule the period respects is weaker than it looks: the matrix's four patterns are spatial, a beam going round and a column stepping across, where the breath has no spatial structure at all.
