...
| `Colour bar` | 行首竖条，与行块前缘齐平 | `2 × 40`、圆角 `1`，在 `80` 行内垂直居中，行内 `x = 0`（距面板边 `12`），取产品点亮色 `#6CB4FF` / `#D97757`；说明行退回纯 Project 文本 |
...
竖条画在行块的**前缘**（行内 `x = 0`），因此行块的缩进就是竖条的位置。它一度沿用行块原本的 `6`，代价是竖条比上方的状态矩阵、下方的额度规则都朝里半个 `12`——面板上唯一一条不与其余竖直边对齐的线。现在只在竖条画出来的形态里把行块缩进改成 `12`（行内边距不动，仍是 `6`），竖条落到那条共用边距上，说明行仍保持整宽，行内文字退到 `18` 站到竖条后面。单产品时不画竖条，行块回到 `6` + `6`，文字仍落在 `12`：多出来的那 `6` 是竖条自己的地方，没有竖条就不该占。高度取行高的一半，避开行块自己的 `12` 圆角。`onlyTheTwoNamingStylesPutTheProductOnTheCaption` 锁定四个选项里只有前两个把产品名写进说明行——写错会让产品出现两次，一次是词，一次是条。
