...
**The two wings move on one rule: each is exactly as wide as what it draws.** The notched bar reserves nothing, on either side — and the notch-less pill's trailing slot has since joined it, so a reading now costs the same and moves an edge on both collapsed forms (§6.4). A session column opening widens the leading wing; a digit, a badge, or a badge's own digit widens the trailing one; each closing gives that width straight back. Because the panel is pinned to the cut-out (§3.4), the leading wing can only take its width **leftwards** and the trailing wing only **rightwards**, and neither side can reach the other — the trailing wing is a whole number of points and cancels out of the sum that places the leading edge, and the leading wing is not a term in the trailing edge at all. Pinned by `theNotchedBarIsExactlyAsWideAsWhatItDraws` and `theCollapsedWingsMoveOnlyWhenTheirContentsArriveOrLeave`.
