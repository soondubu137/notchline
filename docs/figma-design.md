...
- [x] An idle mark glimmers once when the panel opens and holds still otherwise, over `2 s`, peaking at `0.8`, with the two products together (§4.1). Pinned by `eachStateDrawsThePatternItsDesignFileDraws`, `twoIdleMarksGlimmerAsOne`, `anIdleMarkGlimmersOnceForAnOpeningRatherThanOnALoop` and `anIdleMarkDoesNotGlimmerUnderReduceMotion`. Onboarding's legend still teaches five swatches, not six: it teaches what the collapsed bar says, and the glimmer is not something the collapsed bar can say.
