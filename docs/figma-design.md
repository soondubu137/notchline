...
> **Collapsed and expanded overall sizes are no longer tokens; they are compositions.** Earlier revisions of this table carried `348 × 46`, `168 × 46`, `200 × 46`, `520 × 302`, a `256` content region and a `520 × 94` thin state, and every one of those is stale. Collapsed widths are composed by `PanelMetrics` from measured text and rounded up: the notch-less single-product working set is one fixed `209` (so `Running` and `Input needed` are the same width, not `168` and `200`), two products `238`, and `Disconnected` `136` (§6.4). The notched form is two wings plus the occlusion, varying with the cut-out and the menu bar: under a `200 × 46` cut-out, resting is `243` for one product and `271` for two (§3.6 — `246` / `274` date from the wingless trailing step, removed). **Those are the numbers for one scaling step, not for "this machine"** — the cut-out is measured, and another step gives another set: this machine currently measures `220 × 38`, giving `263` / `291` resting and `319` / `347` timed. The first three were `201` / `230` / `136` before the reading's ground (§4.7) and `196` / `218` / `240` before the session-count dots; `Disconnected` is unchanged by both, having neither a dot column nor a reading. **Expanded height is `panelHeight + 240 viewport + footer`**, so at a `46` menu bar it is `316` (Codex only), `339` (Claude Code only), `370` (both) and `308` (quota block folded) — never `302`. Those four were `326` / `340` / `370` / `314` while each footer was a constant of its own; they are now composed from their content plus one shared `6` bottom margin ([`dual-agent-design.md`](dual-agent-design.md) §5.1). The thin expanded state is a `48` body plus the footer, `124` at that height. Read §5, §6.4 and §6.8 for current numbers; these notes exist only to date older files.
...
**The two wings move on one rule: composition, never counting.** The leading wing never moves at all — nothing on the trailing side can reach it (§3.4). The trailing wing moves when a timer or a badge *arrives or leaves*, and at no other moment. Tabular figures were only ever half of that: they hold a reading still inside one digit count and not across them, so `9:59 → 10:00` was `8` pt and `59:59 → 1:00:00` another `12`, taken off the trailing edge — the edge a person reads against the notch — in the middle of a Turn. So the timer is billed for a fixed slot, `PanelMetrics.timerReservationWidth`: the `00:00:00` (Medium) template plus the reading's ground, `65.91`, the same one declaration the notch-less pill reserves. The reading is right-aligned in it, so a short one shows as black between the notch and the digits rather than as a shorter bar. **The cost is stated plainly**: the notched bar is `30` pt wider for the whole of every timed Turn (trailing wing `86` rather than `56` for a `0:00` reading). That is the price of a still edge, and it is only paid while a Turn is timed — the idle notched surface reserves nothing out here, which is where it differs from the pill. A reading past a hundred hours outruns the template and widens the slot; nothing else does. Pinned by `theCollapsedWingsMoveOnlyWhenTheirContentsArriveOrLeave`.
