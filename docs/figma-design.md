...
- Marks are `PanelMetrics.marksWidth(markCount)`, including each product's reserved session-count dot column (§4.6): `22.26` for one product, `50.51` for two. **The expanded header still reserves that column**, and so does the notch-less pill; only the notched collapsed bar composes from what it draws (`drawnMarksWidth`, §4.8). This panel is centred on the display, so every point one side wants is taken from the other as well — a width that answered to the session count would move both edges and everything between them each time a Thread opened.
...
| Leading padding | `12` |
| Status matrix | `16.6` |
| Its session-dot column (§4.6, reserved at every session count) | `5.66` |
| Gap | `12` |
| Widest compact name `Connected`, at the raster's own ceiled box | `67` |
| Trailing padding | `12` |
| **Resting width, one product** | **`125.25` → `126`** |
| A second mark and the `6` between the pair | `+28.26` |
| **Resting width, two products** | **`153.51` → `154`** |

Everything the pill can say falls inside that `67`: `Completed 65.81`, `Approval 52.74`, `Running 48.99`, `Input 30.45`. `Disconnected` is `135.58 → 136`, sized for its own word (see below).

**What widens it, and by exactly what.** The trailing slot is `32` of clearance plus the drawn reading (`PanelMetrics.drawnTrailingSlotWidth`), and that reading is the very expression the notched wing is billed for (`drawnTrailingReadingWidth`, §4.8): the badges, the `8` between them and the timer, and the timer's ceiled glyph box on its `8` of ground. So one product runs `126` resting, `194` at `1:23`, `202` at `10:00`, `214` at `1:00:00`, `173` with a badge and no timer, and `217` with both; two products add `28` to each. Pinned by `theTrailingReadingWidensBothCollapsedForms`, `theRestingPillIsTheMarksAndTheWidestWordItCanSay` and `noNotchCompactIsOneFixedWidthAcrossTheWorkingSet`.
...
>
> **Two things about it did not hold up.** It was `83` of empty pill for the whole of every untimed moment, which is most of this surface's life — the slot and its clearance came to `98`, of which `14.26` comes back as the widest *name* the resting width now holds. And it was bought against a neighbour this window overlaps rather than displaces: nothing in the menu bar is laid out from this panel's frame, so a wider pill draws over an icon and a narrower one uncovers it, but no icon ever moves. What survives is the half that was load-bearing, and it is the resting width above — the pill still cannot answer to *which* state is showing, because that changes several times inside one Turn. It answers to what has arrived instead, which is the one kind of movement this surface has always accepted.

**`Disconnected` is still the one state sized for its own word, and it has changed sides.** Nothing follows it into a reading and it has no product behind it whose sessions could want a column, so it is its own name and the marks and nothing else — the rule is untouched. But `Disconnected` (`82.96`) outruns `Connected` (`66.05`) by more than the `5.66` column a connected mark adds, so at `136` it is now the **wider** of the two resting forms: the pill narrows by about `10` when the first product connects, where it used to widen by `73`. Pinned by `disconnectedIsSizedForItsOwnWordRatherThanTheWidest`.
...
Before the side padding narrowed from `24` to `12`, the reserved composition's three widths were `220`, `242` and `160`; nothing else in that table changed, and the difference was two `12`s.

This also closes the item registered in [`dual-agent-design.md`](dual-agent-design.md) §8: `Update Claude Code` once pushed the two-product pill from `189` to `202`. The whole paragraph is historical now — the label no longer exists, and `updateAgent` says the product-free `Update` / `Update required` collapsed and expanded alike (§6.6) — but the arithmetic is worth dating: at `124.77` it fitted inside the reserved `196`, and it would not fit the resting `126`, because the resting width holds the longest name the set can say and that sentence is not in the set. (Measured here, `Update Claude Code` is `124.77`, matching the `124.8` recorded in §8, which is why the rest of that table is trustworthy.)
...
| --- | --- |
| One Codex turn running, with a subagent stopped on a question | The bar says `Approval`, the blue badge lights, and the first row's reading sits on white |
...
- [x] The notch-less pill is one width across the working set at any given reading, resting at `126` for one product and `154` for two, with `Disconnected` `136` (§6.4) — measured with the system font on this machine. It widens for the timer and for the subagent badges and gives the width back, on `PanelMotion`'s curve. (This line carried the reserved `209` / `238` before the slot went, `196` / `218` before the reading's ground, and `220` / `242` before the side padding narrowed.)
