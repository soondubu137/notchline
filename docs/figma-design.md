...
**This section is the notch-less pill's alone.** The notched bar reserves nothing and is exactly as wide as what it draws, both wings (§4.8); everything below is the argument for the one form that has menu-bar icons to its left and therefore a position to hold.
...
The timer is **right-aligned** in its reserved space with monospaced digits, so a Turn crossing an hour grows leftwards into space that was already empty: the pill does not move, and neither do the menu-bar icons to its left. The notched bar does the opposite and for the same reason read the other way — it has nothing to its right to disturb, so the reading is drawn from the leading edge of a box that is exactly its own width, and the panel edge opens rightwards to let the new digit in (§4.8).
