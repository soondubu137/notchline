...
| `13 — State matrices: the four marks` | The four collapsed Turn statuses' animated marks (Radar / Double Knock / Advance / Lull), 4×4 grid, `16.6` footprint unchanged. The Glimmer (§4.1), which an idle mark draws only while the panel is open, is not on this page — `design/assets/matrix-states/` governs it | `920:2` |
...
| Glimmer | Connected, **panel open** | `2 s` (60 frames), **played once per opening** | A soft highlight drifts over the mark: a Gaussian `1.25` cells wide whose centre travels a 1:2 Lissajous figure, `1.6` cells of travel from the mark's centre in each axis with the horizontal quarter-turn skewed `1.0 rad`. A cell is as bright as the highlight is close, peaking at `0.8` rather than at the full the other four reach | `0.15` |
| (still) | Connected (panel shut) / Disconnected | — | Does not move | `0.15` |

**A mark fades between the still and a pattern, and cuts between two patterns.** Going dim to lit and lit to dim is the mark starting or stopping having something to say — a turn beginning, a finished one being read, the pointer arriving on an idle mark. Cut, those read as a light being thrown: louder than the quiet events they carry. So the two lit stacks are crossed over on the panel's own curve (`PanelMotion`, `0.20 s`, `0.08 s` under Reduce Motion, which shortens the hand-over rather than removing it — a dissolve is what movement is replaced *with*), and the pattern comes up out of the still and goes back down into it. **The unlit bed is no part of the fade**: it draws the same sixteen cells whatever the pattern is, so fading it would fade the mark itself away rather than the pattern on it. **The pattern leaving goes on running while it goes**, both stacks drawing for the length of the fade, so a completed mark's crest is still crossing as it dissolves rather than freezing on a frame and then vanishing. **A change between two live patterns is cut** — Running to Completed, Input to Approval — because the product is saying a different thing and there is no moment at which it is half of each; it is also the fade the eye would read least, both sides being lit and moving. `MatrixIndicatorView.dissolves(from:to:)` is the rule, pinned by `aMarkFadesBetweenTheStillAndAPatternButNotBetweenTwoPatterns`.
...
**A connected product with nothing running glimmers while the panel is open.** It is the only treatment that is not a reading of the product — every other one says what a Turn is doing, and this one says the mark has been looked at. The compact bar is unchanged: an idle mark there is the still, because a mark on the menu bar is glanced at and must not move for its own sake. Opening the panel is a deliberate act, and a mark that answers it is a mark the user can tell is live rather than painted on. The glimmer is normalised to the still's own level (`0.15`) with its highlight peaking at `0.8`, so waking the mark lifts it out of exactly where it was resting and drops it back; a cell the highlight is nowhere near is drawing what it drew before the pointer arrived. **The ceiling is `0.8` and not `1` on purpose**: the other four are readings of a product doing something, and at a shared ceiling the mark with no news was the brightest thing on the bar. Four fifths of the way up is plainly lit and plainly below every state that has something to report, so whatever is brightest on the surface is always something with a reason to be. Nothing else changes — a product with a Turn in flight already has something of its own to say and keeps saying it. **Motion off keeps the still**: a single frame of a wandering highlight is a scatter of brightnesses saying nothing, and a surface asked to hold still acknowledges the pointer by holding still. **The resting grey never glimmers** either — its two colours are the same colour, so a curve through it would move nothing but the glow, and it exists to say nothing can happen.

**The glimmer plays once per opening, not for as long as the panel is up.** It is the mark answering the pointer, and an answer is given once. Repeated it becomes a beat — the one thing this figure is shaped not to have — and it goes on costing the render server for as long as a panel held open by hover stays open, which can be minutes. So the animation is given `repeatCount` `1` and starts when the pointer arrives rather than on the shared period grid the four looping patterns use: a one-shot anchored to that grid would begin somewhere in the past and draw whatever fraction of itself was left. One turn round the figure says the mark is live; after that an idle product is drawn the way an idle product looks. **The turn takes `2 s`, having started at `4`.** Drawn once rather than looped, four seconds was longer than a panel opened to read something is often up for: the pointer left while the highlight was still crossing, so what a user saw was a fragment of a wander rather than a figure. At two the whole figure fits inside a glance and nothing of the drift's character is spent — the highlight is as soft and travels as far, so it reads as the same unhurried thing said in half the time. It also puts the mark's two slowest patterns on one length, the pairing `MatrixPattern.period` already makes for the radar and the knock. `NotchStatusMatrix.glimmerHasRun` is what notices the turn is over — local to one mark, moving once, redrawing that view and not the surface — and closing the panel clears it, so the next opening is answered as freshly as the first.

**The two products' glimmers are staggered a third of a loop, and a third rather than a half.** Every other pattern is deliberately in sync across marks, which is what `MatrixIndicatorView.phaseAnchor(for:)` is for: two products running one curve out of step read as noise rather than as one state said twice. The glimmer is the exception, because it is the pattern that says nothing is happening — two idle marks rising and falling as one would be a rhythm, and a rhythm is what it is careful not to have. (Running once, both marks do start together, on the opening that woke them; what keeps them apart is the stagger in the curve, which is where it always was.) Half the loop is the obvious stagger for two products and the one value that fails here: the figure crosses the columns twice for every once it crosses the rows, so half a loop moves the highlight only in the rows, and because the mark is symmetric top to bottom the two marks' total brightness would then match frame for frame. A third is coprime with that doubling, so neither the figure nor its half repeats between the marks, and it holds for a third product too. The stagger goes into the track rather than into the animation's `beginTime`, exactly as a cell's own phase does, so the shared clock survives intact and what differs between two marks is the curve each draws.

**The glimmer is held as its rule, not as a curve.** The other four each write one waveform sixteen times at sixteen offsets, because their highlights travel in one dimension — round a bearing, across a column, along the diagonal — so the code holds the waveform and computes the offset. This one travels in two, so a cell in the middle of the figure is passed close and often while a corner is passed far and rarely, and the sixteen curves genuinely differ in shape. What survives being read is the figure, so `MatrixTrack.glimmer` writes the figure down and samples the curves off it, rounded to the same three decimals the SVG writes. `notchline-connected-glimmer.svg` was regenerated when the ceiling came down, so the file and the code are still the same numbers rather than merely close ones. The generator for all of them is `loaders-wtf-import.js` in the same folder; the glimmer's entry there is normalised by its own floor and swing (now `0.15 + 0.65`) rather than by that file's `norm()`, which lifts a ceiling but leaves a floor where it found it.

**Each treatment has its own darkness, so readability no longer requires deviating from the design files.** The previous version's two tracks (waiting and terminal) were changed in code to bottom out at `0.100` rather than the file's `0.200`, because the file rested every dark cell at one level and "waiting for you" was as black as "nothing running". These four files rest at three different levels — `0.05` for knock and advance, `0.139` for radar, `0.182` for lull — with the resting level threaded between them, so darkness alone still separates a mark waiting on the user from one that is merely present, and that deviation was removed entirely. The glimmer is outside this ordering by design: it is not a Turn state, it is the still with the pointer on it, and its floor is the resting level itself.

**Resting moved from `0.18` to `0.15`.** That level comes from no design file — there is no still frame in `matrix-states/` — so it follows the live tracks. (The glimmer is the one that follows *it*: its floor was chosen so that waking an idle mark starts and ends at the level it was already holding. Moving the resting level again would mean regenerating the glimmer with it — the swing is written as `peak − floor`, so only the file needs redrawing.) `0.18` was set while lull still rested at `0.343`, and the redrawn lull's trough is `0.182`, making "this Turn ended" and "nothing is running" equally dark at the trough. Lowering it restores the ordering: both waits' `0.05` below it, lull's `0.182` above it, radar's `0.139` below. Only the first of those three gaps is a darkness difference the eye genuinely reads — a mark waiting on the user is three times darker than resting, and that is the pair that must never be confused. The other two are ordering and need not carry weight on their own: neither radar nor lull ever has all sixteen cells at the trough, so a live mark always has a lit cell while a resting one has none. What `eachStateDrawsThePatternItsDesignFileDraws` pins is that ordering, not these three numbers.

**The seam direction now has other witnesses.** The four old patterns were vertically symmetric, so a wrong `isFlipped` showed only on onboarding's diagonal (§7). Three of the four new ones are asymmetric: radar sweeps backwards, advance's bottom row moves to the top, and lull walks the wrong diagonal. `theSplitMatrixCutsOnTheSameDiagonalAsTheMark` is still the most direct assertion, and `eachStateDrawsThePatternItsDesignFileDraws` pins all five tracks and their phases cell by cell. The glimmer is no help here — half a loop mirrors it top to bottom, so it draws the same thing either way up — and what that test asserts of it instead is the mirror itself, which no figure but a 1:2 one produces.
...
Implementation: `MatrixGrid` (geometry), `MatrixTrack` (the five curves and four phase rules), `NotchMatrixState` (status), `MatrixPattern` (what is drawn — the two are separate types because the glimmer depends on the panel and the panel is not a reading of any product) and `MatrixIndicatorView` (layers and glow), all in `NotchStatusMatrix.swift`. The panel's own state reaches the mark through `StatusReadout.isPanelOpen`.
...
- [x] An idle mark glimmers once when the panel opens and holds still otherwise, peaking at `0.8`, with the two products a third of a loop apart (§4.1). Pinned by `eachStateDrawsThePatternItsDesignFileDraws`, `twoIdleMarksGlimmerOutOfPhaseWithEachOther`, `anIdleMarkGlimmersOnceForAnOpeningRatherThanOnALoop` and `anIdleMarkDoesNotGlimmerUnderReduceMotion`. Onboarding's legend still teaches five swatches, not six: it teaches what the collapsed bar says, and the glimmer is not something the collapsed bar can say.
