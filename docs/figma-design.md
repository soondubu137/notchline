...
| Glimmer | Connected, **panel open** | `4 s` (120 frames), **played once per opening** | A soft highlight drifts over the mark: a Gaussian `1.25` cells wide whose centre travels a 1:2 Lissajous figure, `1.6` cells of travel from the mark's centre in each axis with the horizontal quarter-turn skewed `1.0 rad`. A cell is as bright as the highlight is close, peaking at `0.8` rather than at the full the other four reach | `0.15` |
