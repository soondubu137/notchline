...
| Field | Value |
| --- | --- |
| Status | **All three stages are built.** §14.1 landed on 2026-09-05, §3 to §8 with it, and **§9.3 — the chord and the keys under it — on 2026-09-06**: a request now arrives, is read in the notch, and is answered there with the pointer or without one. §15 q01 and q02 came back yes, and q14 — *when does the keyboard half land* — is answered by its landing. Two clauses of §9.3 changed while it was built and are corrected in place: an arrow is the caret's while the caret has somewhere to go (§9.2), and the body's keyboard scrolling is a running total rather than one press's delta (§6.2). Nothing here is deferred any more; what is *not* offered is stated where it was always stated — `Always` (§6.5), a plan's mode (§4.3) and an MCP elicitation (§2.2). |
| Version | 1.4 |
| Date | 2026-09-06 |
...
~~**Stage 02 is the initial version of this experience, and stage 03 is deferred.**~~ **All three are built** — stage 03 on 2026-09-06, and it was a build rather than a second design, which is what keeping §9.3 whole was for. The one thing the writing did not foresee is in §9.2: the arrows cannot simply be taken from the field, because the caret is in that field from the moment the row opens.
...
At rest the mark says `Approval needed` or `Input needed`. Under the pointer it says `Answer` where the request can be answered here and `Read` where it cannot (§11). **Built 2026-09-05**, and two things about it are worth recording. The pointer that matters is the one on **the mark**, not anywhere on the row: the text is the Thread and the mark is the request, so a pointer resting on the title must not describe what the mark would do. And the word reads the row's **own** turn rather than its derived status, so a `Working...` row whose subagent is waiting keeps its timer and says so in brightness alone — which is `CONTEXT.md`'s rule that a row reports one Turn, with the ground as its single exception. **The ground does not resize** — only the word inside it changes, so nothing on the row moves under a passing pointer. Which settles what the ground says at rest: it is sized for the longest name it can hold, so [`figma-design.md`](figma-design.md) §4.7's *reading* — a duration — cannot be what is in it, or `Answer` would not fit without moving it. The duration keeps the place it already had, on the finished row's dark ground ([`panel-v2.md`](panel-v2.md) §3.5).

~~Drawing the word under keyboard focus as well as under the pointer would be **a correction to §6** of the superseded document, and it is **not made in the initial version**~~ — **made on 2026-09-06 with the keys that create the focus** (§9.3). The walk lands on a row, that row takes the list's own hover fill, and its mark says `Answer` or `Read` exactly as it does under the pointer: a keyboard user would otherwise never see the affordance they are about to use. It is read from the store rather than from a view's own hover state, because a walk is a fact about the list where a pointer is a fact about one view — and both draw the word through one expression, so the two cannot drift apart. Measured on Release: with the walk on the first row its mark reads `Answer` while the row below it reads `Input needed`, and one `↓` moves both.
...
**What a command's body actually holds, decided while building it.** Not one field of `tool_input` — every tool names its arguments differently, so reading `command` covers `Bash` and leaves every other approval blank, and "the longest string" is the kind of heuristic §4.6 exists to refuse. And **not the JSON either**: the first build drew the encoder's output, so every approval opened onto its own braces, quotes and escapes before it opened onto its request. What is drawn is every argument, in a fixed order, one per line as `name  value` — with a lone argument drawn bare, since most tools have one that matters and naming it is noise at exactly the wrong moment. Nothing is omitted and nothing is annotated; the wrapper the transport happened to use is simply not part of the request.
...
With `multiSelect` the ground **starts on `Send`**, the numerals become `12 × 12` boxes at corner `4` — `#242424` empty, the theme ink's lit value filled — and a click on a box or its label ticks it. ~~and never leaves it~~ was true of the pointer version and is not of this one: since §9.3 landed an **arrow** moves the ground onto an option, where `Space` ticks it and the ground stays where it is — which is this line's own parenthetical, now that the keys exist. What the rule protects is unchanged and is about the *default*: the ground **begins** on `Send`, so the person most likely to press `⏎` twice is not pressing it on a toggle. The alternative, `⏎` toggling and something else sending, would make the brightest object on the row stop being what `⏎` does, on the one form where a person is most likely to press it twice.
...
**The walk clamps rather than wrapping.** `→` on the affirmative is not `Deny`: a row offers two to five answers, which is short enough that reappearing at the other end reads as a jump rather than as a step.
...
### 6.2 What scrolls the body, and how the axes divide

The pointer's wheel scrolls a body taller than the space it has. **And since 2026-09-06 so do the arrows**, on the rule this section always named: on a row whose body holds **options** all four move the ground and the body scrolls to keep it in view, and on a row whose body holds **no answers** the axes divide the way the drawing already does — `←` and `→` between the two controls, `↑` and `↓` down the body. With no row open the list is what the arrows walk, and it is still browsed by hover exactly as it always was.

**The keyboard's scrolling is a total, not a nudge, and that correction was measured rather than reasoned.** The body owns its own offset — publishing it would put a whole-panel re-render behind every wheel event (`system-architecture.md` §6) — so what the store publishes is what the *keyboard* has asked for, and the body applies it. Published as one press's delta, **four `↓` moved the body a single line**: SwiftUI compares the value once per render pass, and four presses inside one pass are one change, so three of them were lost. Published as a running total, the body applies the difference from the total it last saw and a burst cannot lose anything — measured on Release, `+54` lines under the fold falling to `+50`.
...
| `↑ ↓ ← →` | Move the caret, and **the ground where the caret cannot move** | §9.3, corrected below |
| `1`–`4` | Take the option they number, **while the field is empty and the ground is still on one** | Otherwise a character like any other (§15 q08) |
| `Space` | Tick the option the ground is on, on the one form that ticks, and on the same condition as the digits | Otherwise a space (§5.5) |
...
| everything else | **Unbound** | Binding a key with nothing to do would be a keyboard model with a hole in it, which is worse than a panel that plainly does not navigate |

**The keys are still the field's, and that is a correction to §9.3 made while building it.** §9.3 gives the arrows to the white ground outright; this table's own rule is that the panel's keys are the field's own, and the caret is in that field from the moment the row opens (§6.6). Both cannot be true: an arrow bound to the ground cannot put the caret back inside a two-line refusal, and a refusal of two sentences is exactly what `⇧⏎` exists for. So **an arrow is the caret's while the caret has somewhere to go, and the ground's when it has not** — `←` at the start of the text and `→` at its end move the ground, as do `↑` and `↓` on the first and last line. An empty field is both edges at once, so it costs the ordinary case nothing; a full one costs one extra press. It is the ordinary behaviour of a field standing in a strip of controls, and it needed no new drawing.

The same rule settles the other two: a digit and a space are characters the moment there is anything in the field, which is §15 q08's answer applied to `Space` as well — a multi-word answer must not lose its gaps to a control nobody asked for. `⌘⏎` and `⇥` are bound to nothing at all, measured on Release: they send nothing and move nothing, and `⇥`'s only effect is the tab it types.

### 9.3 The keyboard half, built 2026-09-06

Stage 03 of §1.1. It added no region, no ink, no control and no height; it added keys, and one word this surface already drew under the pointer — which is what keeping this section whole through stage 02 was for.

| Key | Does | Argued in |
| --- | --- | --- |
| `⌥Space` | Brings the panel down already latched, with the first row by the existing sort already open and the caret in its field. With nothing waiting it brings the same panel down latched at the top of the list instead, so **the chord is never a key that sometimes does nothing** — the only variable is whether a row is open when it lands. User-settable, defaulting to `⌥Space`, and registered through the ordinary system path so that a clash **fails loudly at registration** rather than quietly at use; the settings row shows the chord it actually holds, not the one it asked for | §15 q03 |
| `↑ ↓ ← →` | Move the white ground, and walk the list where no row is open. The body scrolls to keep the ground in view; on a row whose body has no answers to move between, `↑ ↓` scroll that body directly. **The caret has them first** — §9.2 | §6, §6.2 |
...
| `Space` | Ticks the option the ground is on, `multiSelect` only, on the digits' own condition | §5.5 |
| `⇥` | **Still unbound.** Now that the arrows exist, every journey `⇥` would serve already has a key | §15 q10 |

Three clauses of this document arrived with those keys, and each is a correction only a keyboard user makes necessary: the word drawn under focus as well as under the pointer (§3.3), the arrows as a second force on the ground (§6), and the axis split on a body with no answers in it (§6.2). Two of the three are corrections to [`expanded-panel-v2.md`](expanded-panel-v2.md) that stage 02 deliberately did not make.

**Four things were decided while building it**, none of them visible from the design side:

- **An arrow is the caret's first.** §9.2, and it is the only clause of this table that is not literally what it said.
- **`⏎` on a walked row takes whichever of its two targets that row has.** The row's text is the Thread and the mark is the request (§3), and a single focus cannot mean both — so the walk is on the mark, which is why the mark draws its word under it. A row with a mark opens here; a row without one has only the Thread to lead to, and `⏎` is then the click that has always worked.
- **A row already open stays open when the chord lands on it**, which is this table's own sentence read as a rule about what the chord may take away: opening the first waiting request over an open one would throw away a part-answered set and re-arm an affirmative the reader had already waited out.
- **The chord pressed again hands everything back.** The same exit `⎋` takes, from the key that opened it: a chord with no way out of what it started would be the one key on this surface that can only be undone with a mouse.
- **`⎋` on a chord-opened row keeps the keyboard**, and the second `⎋` closes the panel — which is what §9.2's table always said and what a panel with no pointer on it requires. A first `⎋` that unlatched would strand the panel open with no way to shut it but a mouse. The walk is left standing on the row that was open, so `⏎` reopens what `⎋` shut.

~~**What waiting for this costs is in §13.3.**~~ Nothing waits for it now; §13.3 records what the pointer-first ordering cost while it stood.
...
~~**What the initial version does not give is full keyboard access.**~~ **It gives it now** (2026-09-06). While the pointer half stood alone, somebody who works without a pointer answered in the product exactly as they did before — nothing got worse and nothing on this page was for them — and that was the largest cost of the ordering in §1.1, which is why §9.3 was deferred rather than dropped. With the chord, the walk and the arrows, every state on this surface is reachable and every answer takeable without a pointer: measured end to end on Release, a two-question set was opened, answered and sent with the keyboard alone, and the panel handed the application back afterwards.
...
| 03 | Which chord, and what happens when it is taken? | **Answered by the board's owner on 2026-09-05 and built on 2026-09-06.** `⌥Space`, user-settable, registered with `RegisterEventHotKey` — the ordinary system path, which **answers**: it returns a status, where a key monitor would simply never fire. A refusal is drawn in the settings row, which shows the chord the app actually holds and names the failure rather than the request. The preference stores a **position on the keyboard** and the row draws it in the letters of whichever layout is in front, so a chord recorded on one layout is not mislabelled on another. Still worth testing against a machine already running a launcher on that chord |
...
| 14 | When does the keyboard half land? | **Answered by landing: 2026-09-06**, one day after the pointer half. §9.3 is what it was built from, and it was a build rather than a second design — four decisions were taken during it and are recorded there, of which one (an arrow is the caret's first) corrects the table itself |
...
- [x] Opening moves one bright ground from the caption line to the answer row — the surface never draws two at once — and a chevron pointing up stands where it was. **Counted on Release** rather than eyeballed: flood-filling every near-white region of the panel finds exactly **one** before opening (the mark, `112 × 15` on the caption line) and exactly **one** after (the affirmative, `75 × 27` on the answer row) — the ground travelled and grew, and at no point are there two.
...
- [x] Prose draws with no ground and machine text on `#242424`, chosen by payload and not by length, on all seven shapes in §2. **Sampled on Release**: a command's body is `(36, 36, 36)` — `#242424` — against the open row's own `(43, 43, 46)` around it, and a document, a question with options, a multi-select and an option-less question all draw the body on the row's own ground with no recessed step. **Length plays no part**: a four-line command is machine text and a sixty-line document is prose.
...
- [x] ~~The only keys the panel answers to are the field's — anything printable, `⇧⏎`, `⏎` — and `⎋`. `⌥Space`, `↑ ↓ ← →`, `1`–`4`, `Space`, `⌘⏎` and `⇥` are all unbound, and the app registers no global hotkey.~~ **Superseded by §9.3 landing** — the chord, the arrows, the digits and `Space` are bound now, each on the condition §9.2 states, and the lines below re-verify them. What this line pinned about stage 02 was true when it was written, and the `⌘⏎` and `⇥` half of it still is. Kept as the measurement it was: Measured on Release: `⌘⏎`, `⇥`, the arrows and `Space` send nothing and move nothing; inside the field they do what a field does, which is §9.2's rule rather than an exception to it. **Re-measured 2026-09-05** on an open question set with the modifiers actually applied: across `⌘⏎`, `⇥`, all four arrows, `Space` and the digits, **zero answers were taken and nothing was sent**, and the set never advanced. `⇧⏎` put a newline in the field and sent nothing; the `⏎` after it took the answer and drew the next question. `⇥`'s only effect is the tab it types, which moves the ground exactly as any other character does.
...
- [x] **The chord brings the panel down already latched, on the first waiting request, with the caret in its field** — and with nothing waiting, latched at the top of the list. **Measured on Release** against a staged build: the panel went from `334 × 38` collapsed to `530 × 300` with the approval open, its body drawn, the ground on `Approve` and the caret in the field, with no pointer involved at any point. Pressed again it went back to `334 × 38` and the foreground returned to the application that had it.
- [x] **`⎋` collapses the row and leaves the panel key; the second closes it and hands the application back.** **Measured on Release**: after the first `⎋` the panel stayed at `530 × 300` with the walk standing on the row that had been open, and after the second it was the collapsed bar again with `lsappinfo front` naming the application that was in front before the chord. While the row was open that reading named Notchline, which is ADR 0020 exactly.
- [x] **The walk draws the list's own hover fill and the mark says its word under keyboard focus** (§3.3). **Measured on Release**: with the walk on the first row that row carried the fill and its mark read `Answer`, while the row below it read `Input needed`; one `↓` moved both, and the mark did not resize as the word changed.
- [x] **The arrows move the white ground, and the caret has them first** (§6, §9.2). **Measured on Release**: `←` on an approval with an empty field moved the ground from `Approve` to `Deny` and neither control moved; with `x1` typed, two `←` walked the caret through the text with the ground still on `Send`, and the third — the caret already at the start — moved the ground onto option `3`.
- [x] **A body with no answers in it scrolls on `↑ ↓`, and a burst of presses is not lost.** **Measured on Release**: four `↓` over a `60`-line command moved the body four lines, `+54` under the fold falling to `+50` with the rail's thumb travelling. It moved **one** line before the nudge became a running total, which is §6.2's correction.
- [x] **A digit takes the option it numbers until something is typed, and is a character afterwards.** **Measured on Release**: `3` on a three-option question took `DuckDB` and drew question `2/2`; after `x` was typed, `1` went into the field as `x1` and took nothing.
- [x] **`Space` ticks the option the ground is on, `multiSelect` only.** **Measured on Release**: on question `2/2` the ground began on `Send`, `↑` moved it onto `Render`, `Space` filled that box, and the ground stayed on the option rather than jumping back.
- [x] **A whole question set can be answered with the keyboard alone.** **Measured on Release, end to end**: chord, `⎋`, `↓`, `⏎`, `3`, `↑`, `Space`, `↓`, `⏎` sent `answers([Which database → DuckDB, Where should it run → Render])` on the ticket the request arrived on, and the next waiting request opened itself unarmed.
- [x] The band, both collapsed forms and the quota footer draw identically to pages 01 to 09 in every state above. **Measured rather than eyeballed, 2026-09-06**: the same staged fixture — two waiting rows and a quota — was built at `99d544c` and at this change, launched on Release, and screenshotted collapsed and hover-expanded by window id. The two collapsed captures and the two expanded captures differ **only inside `(29, 17)–(71, 59)` in the 2× capture**, which is the aggregate mark's own bounding box: the same `5 × 5` grid at the same position and size, caught at two phases of the `CAAnimation` it has always run. Every other pixel is identical — both wings, the count beside the mark, the rows, the seam and the quota footer — and the panel is `334 × 38` collapsed and `530 × 220` expanded on both builds. Hover also still browses rather than latching, which this capture is the proof of: the expanded panel it took has no walk drawn on it.

## 17. Implementation mapping

**Built** (2026-09-05): a request is read in the notch and answered there, in every shape either product asks in. The work landed in a handful of places, and the first of them was the only one with no dependency outside this repository. **The keyboard half followed on 2026-09-06** and added one file, four published properties and one settings row — everything else it needed was already drawn.
...
| `KeyChord`, `PanelChordHold`, `PanelChordHolder` | **Built 2026-09-06** (`PanelChord.swift`). One key and its modifiers as a value, what the system actually gave the app for it, and the `RegisterEventHotKey` registration that answers. The preference stores a **position** (`keyCode:modifiers`) and the drawn form is asked of the current keyboard layout through `UCKeyTranslate`, so a chord recorded on one layout is not mislabelled on another. A chord with no modifier is refused at the recorder: a bare global hotkey takes that key from every application on the machine |
| `MonitorStore` — the keyboard | **Built 2026-09-06.** `isKeyboardEngaged` and `walkedRowID` are the chord's own latch and the walk's position; `isLatched` is now *either door* rather than `openRowID` spelled differently, because with nothing waiting the chord latches a panel with no open row on it. `takeTheChord()`, `takeArrow(_:)`, `takeTheWalkedRow()`, `takeNumberedOption(_:)` and `tickTheGroundedOption()` are the whole surface of it, and `answerWalk` is the order the ground moves in. `bodyScrollNudge` is a **running total** rather than a delta (§6.2), and `AnswerProgress.ground` is what an arrow sets — the ground stays derived until somebody moves it, and typing clears it again |
| `OverlayPanelController`, `OverlayPanel` | **Built 2026-09-06.** The controller owns the `PanelChordHolder` — a registration is a live system resource rather than a value the store can hold — re-asks for it when the preference changes, and publishes the answer back through `MonitorStore.recordChordHold(_:)`. The latch now merges `$openRowID` and `$isKeyboardEngaged` and reads `store.isLatched` after the hop, because `@Published` emits from `willSet`. `OverlayPanel.handleKey` is the walk's own keys, reached only where no field holds the caret |
| `AnswerFieldView` | **Built 2026-09-06, and it is where §9.3's one correction lives.** The arrows, the digits and `Space` are read off the event: an arrow is handed to the ground only where the caret has nowhere to go — `←` at the start, `→` at the end, `↑ ↓` on the first and last **line fragment**, asked of the layout manager rather than of the string so a wrapped line still counts as a line. The digits and `Space` are taken only bare and only with an empty field |
| `AnswerChordRow` (`SettingsWindow.swift`) | **Built 2026-09-06.** The `Keyboard` group's one row: the chord the app holds, a recorder that arms a local key monitor rather than taking first responder, and a status line that says *held* or names the refusal. It is the only setting in that window that can fail, which is most of why it earns a card of its own |
