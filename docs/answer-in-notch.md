...
Only `Send` or return records the current answer. Choosing an option never submits or advances. On an intermediate question, Send draws the next question and updates the count; on the last, it sends the complete set once. Advancing clears the field, selection and expanded descriptions. Collapsing and reopening the same request preserves them. A replacement request in the same Thread starts fresh.
