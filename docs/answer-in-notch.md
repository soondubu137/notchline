...
The local CLI uses the existing request boundary and answer registry. Ordinary `PermissionRequest` grants/refusals travel over the held native Hook connection; an answer alone never changes Turn status. Synchronous `request_user_input` is waiting and reading-only. Asynchronous questions and permission variants retain their separate, limited coverage in [product support](product-support.md). Exact `Interrupt` clears the observed Turn's waits. Losing an execution withdraws only its owned rows/handles; a competing executor cannot substitute a handle for the same Turn.
