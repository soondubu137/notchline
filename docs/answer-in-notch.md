...
| `NotchOverlayView` | **Started.** `SessionStatusControl` now draws the status name on a ground sized once for every word it can hold, with its own hover so `Answer` / `Read` answers for the mark rather than for the row — and the layer-backed readout it replaced on those rows took a redraw a second with it. Still owed: `OpenRow` and its four bodies, `OptionRow`, `AnswerRow`, the chevron in the trailing head slot, the mark's second hit region, and a hit region and hover fill on every answer (§6.6). The field is an `NSViewRepresentable` over an AppKit text view (§13.2) |
| `PanelMetrics` | **Started.** `waitingMarkWidth`, `waitingMarkFont` and the two words. Still owed: `requestBodyMaximumHeight = 140`, `openRowHeight(bodyHeight:)`, `optionRowHeight = 24`, `answerRowHeight = 28` |
