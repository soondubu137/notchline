...
### 5.4 Typed text replaces the selection
...
A single-choice click replaces the current selection; a multiple-choice click toggles that option. Both begin without a selection and keep the bright submission ground on Send. Without typed text, Send sends the selected label, or the selected labels joined with `, ` in their original order. With typed text, §5.4 applies to both modes.
...
| `1`–`4` | Selects the corresponding single-choice option while the field is empty | Never submits. Multiple-choice forms and non-empty fields receive the digit as text |
| `←` `→` | Walks a question set backwards, and forwards again over a question already reached — **while the field is empty** | The digits' own condition, and for the digits' own reason. Never submits: `→` cannot reach a question the set has not drawn (§5.7). Over text the arrow is the caret's, exactly as below |
...
**The digits are offered to the panel only without modifiers.** The store accepts them only on a single-choice question with a completely empty field and a corresponding option. They use the same selection action as a click. `⌥1` and digits in a non-empty field remain text.

**The two horizontal arrows are offered on exactly that condition** (§5.7, 2026-09-07), and the vertical pair still is not. A bare `←` arrives as `moveLeft:` — `⌥←` is `moveWordLeft:` and `⌘←` is `moveToBeginningOfLine:` — so the selector *is* the bareness test and no modified arrow ever reaches the panel. The store then takes it only while the field is empty, which is the one state in which a caret has nowhere to move anyway, and declines it otherwise, whereupon the text system keeps the key and edits as it always did.

**What that condition costs, stated plainly: the arrow gives up on a question whose answer was typed.** Walk back over a question holding text and `←` is the caret's there, so the set stops moving on that key. `Back` has no such condition and is what works in every state — the arrow is a shortcut over the control, and never the only way. Binding the arrow unconditionally was the alternative and it is refused for the reason this whole table exists: it would take the caret's own key off a person revising a sentence they are part-way through writing.
...
- [x] The only keys the panel answers to are the field's — anything printable, `⇧⏎`, `⏎`, `1`–`4` on an untouched question — and `⎋`. ~~`⌥Space`, `↑ ↓ ← →`, `Space`, `⌘⏎` and `⇥` are all unbound~~ **`← →` came out of that list on 2026-09-07** (§5.7): on a question set with an empty field they walk the set, and everywhere else they are still the caret's. `⌥Space`, `↑ ↓`, `Space`, `⌘⏎` and `⇥` are unbound, and the app registers no global hotkey. Measured on Release: `⌘⏎`, `⇥`, the arrows and `Space` send nothing and move nothing; inside the field they do what a field does, which is §9.2's rule rather than an exception to it. **Re-measured 2026-09-05** on an open question set with the modifiers actually applied: across `⌘⏎`, `⇥`, all four arrows, `Space` and the digits, **zero answers were taken and nothing was sent**, and the set never advanced. `⇧⏎` put a newline in the field and sent nothing; the `⏎` after it took the answer and drew the next question. `⇥`'s only effect is the tab it types, which moves the ground exactly as any other character does.
...
| `AgentRequest`, `AgentRequestReading` | **Built**, and not in the original plan. The typed request and its four drawn forms, read **in the reducer** at the moment the wait opens — the one place holding the signal, the event name, the tool name and the vocabulary at once. Approval arguments retain their field names, values and typography roles in `argumentFields`, projected from `tool_input` in sorted key order (§4.2); question options retain their full title and description for the measured card layout (§5) |
...
| `AnswerField.onQuestionStep`, `AnswerFieldView` | **Built 2026-09-07.** `moveLeft:` and `moveRight:` are offered to the store from `textView(_:doCommandBy:)` — where `⏎` and `⎋` already arrive — rather than read off the event like the digits, because the selector *is* the bareness test here: `⌥←` is `moveWordLeft:` and `⌘←` is `moveToBeginningOfLine:`, so no modified arrow can reach the panel at all. The store declines unless the field is empty and `false` hands the key straight back to the text system, so every arrow over text still edits it |
