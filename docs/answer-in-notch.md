...
- **The mark is drawn as a control now rather than as a reading**: ~~`32` tall — twice the reading ground it used to borrow, on that ground's own `0.25` corner ratio — with `12` of padding and `13 pt` Semibold~~ **it is drawn as *the* control — `28` tall on corner `4`, with `12` of padding and `13 pt` Medium, which are `AnswerControl`'s four numbers exactly** — on `NotchPalette.brightGround` with `themeInk`'s unlit end as the ink. ~~Semibold is a correction for the ground rather than a change of emphasis: dark glyphs on a light field read a weight lighter than light ones on a dark field.~~ That correction was right about the effect and wrong about its scope: the affirmative this mark becomes sits on the same ground in the same ink and was never stepped up, so Semibold was not a correction for a light field — it was one control drawn two ways. The height went the same way for the same reason (§3.1). The one thing the mark does not share is the width: an answer hugs its word, the mark reserves the widest of its three, because the mark appears once per row and has a column to draw.
