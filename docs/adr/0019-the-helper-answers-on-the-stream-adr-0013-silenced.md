...
## Local Codex CLI exception (documented 2026-09-22)

The shared definition and helper retain the `wait` reply channel, but production local CLI requests are reading-only. `CodexSurfaceLedger` releases a CLI peer's connection as soon as the payload is read; only a Desktop peer can supply an answer descriptor. Keeping a CLI Hook open suppresses the TUI's own prompt, so the successful isolated approval round trip established transport feasibility but was rejected as product behaviour on 2026-09-15. Resolution still comes from native evidence. This changes no helper bytes or trust definitions; see [request scope](../answer-in-notch.md#local-codex-cli-request-scope-2026-09-15).
