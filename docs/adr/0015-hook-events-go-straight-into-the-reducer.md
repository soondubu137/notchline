...
流式 delta 不在这个投影里，这不是疏漏：把 delta 放进投影就是每秒 3.4 次重画一句用户正在读的话（`system-architecture.md` §6 的实测）。

**但它需要一条自己的边沿，而这条边沿最初开得太窄。** 本文档原先写的是「一行正文由该轮次自身生命周期事件引起的刷新顺带更新，只有『行上什么都没有』那一种需要边沿」。前半句不成立：一次普通工具调用的开合不改投影里的任何字段，所以两次状态变化之间一次刷新都不会发生，行会一直停在上一次刷新时那条消息上。边沿因此改成「**行会画出来的那一行文字变了**」——仍然不是每个 delta 一次，因为 240 字符的头部写满之后，同一条消息后续的 delta 在存进任何结构之前就返回（实测 CLI 2.1.234，一条 1561 字符的消息两次唤醒然后安静）。仍然只为上一次刷新列出过的会话报。详见 `tech-design.md` 第 11 节。
