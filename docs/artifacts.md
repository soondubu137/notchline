...
| Trae’s managed extension storage: `notchline.trae-companion-1.2.1` | The VSIX installed by Trae’s extension CLI in its configured Extensions directory; JavaScript, manifest and the Notchline icon only | Trae’s CLI uninstalls this exact extension; if that CLI call fails, Notchline deletes this exact folder itself |
