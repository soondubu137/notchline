...
**面板在不在屏幕上也走 AppKit 这一侧，不走 view model。** `OverlayConcealmentWatcher` 按 250ms 取一次在屏窗口列表，交给纯函数 `OverlayConcealment.menuBarPresence(onDisplay:windows:otherDisplays:)` 判目标显示器此刻归谁：目标显示器的菜单栏窗口不在在屏列表里就是遮蔽（该屏有应用或视频全屏，或菜单栏设成自动隐藏），在就留在屏幕上。判据只有这一条，所以答案是 `Bool` 而不是「原因」枚举；Mission Control 不隐藏菜单栏，因而落在留在屏幕上那一侧（`PRD.md` §9.2.1）。变了才回调，回调只做三件事——收起（`MonitorStore.collapse()`，被抽走的窗口收不到指针离开）、`orderOut`，或重算 frame 后 `orderFrontRegardless`。遮蔽期间面板照常跟随 frame 变化，只是不重新入屏。判定不出显示器（`DisplayOption.displayID` 为 `nil`，即 `identifier(for:)` 走了兜底那条）时一律不隐藏，理由见 `PRD.md` §9.2.1；**没有屏幕时同样不隐藏**——停表那一侧会把叠层放回屏幕并忘掉上一个答案，见 `system-architecture.md` §7。取样发的号一路带到 `apply(_:ticket:)`，因为判定与上屏之间隔着一次主队列的 hop，而 `sampleNow()` 是在主 actor 上当场上屏的，会插到已经排队的 hop 前面。信号本身与轮询的代价见 `system-architecture.md` §6。
