...
- **带 `agent_id` 的事件不描述本 Thread 的 Turn，一律消费后丢弃。两个产品共用这条闸门，理由不同、代价也不同。** Codex 给子智能体的 hook 盖的是**父 Thread 的 `session_id`** 加**子智能体自己的 `turn_id`**（实测 2026-08-22：子智能体 rollout 的 `session_meta.session_id` 就是父 Thread，它的 stop hook 去解析的也是父 Thread 的 transcript 路径）。于是上一条的接管规则会把子智能体的第一条 `PreToolUse` 当成同一 Thread 的新轮次接管过来，真正的 Turn id 立刻进入 `retiredTurnIDs`，主智能体随后自己的 `Stop` 就被当作退休轮次的迟到事件丢弃——而剩下的那个轮次没有任何东西能结束它。判据只能是 `agent_id`：官方 schema 里 `PreToolUse`／`PostToolUse`／`PermissionRequest`／`UserPromptSubmit` 的 `agent_id` 是**可选**字段，只在子智能体产生该事件时出现。**必须整对丢弃**，`PreToolUse` 与 `PostToolUse` 不能只丢一半，否则 §7.1 那条「只见 close 不见 open」的失信探测会误报。代价说清楚：子智能体自己的审批请求因此不再进入该行的 Approval needed——它此前之所以会进，走的正是上面那条把行毁掉的接管路径。
  Claude Code 侧盖的是**父 Thread 的 `session_id` 加父轮次的 `prompt_id`**（实测 2026-08-23，CLI `2.1.241`：`agent_id` 在它每一条 hook input 都继承的 base schema 上，官方描述为「只在 hook 从子智能体内部触发时出现」）。所以那边接管缺陷不成立，这道闸门守的是另一样东西：`HookTurnState` 只有一格 `openToolUse` 和一格 `pendingApproval`，子智能体的调用是穿过它们的第二股流。**那边的代价更重**：`Agent` 调用期间父轮次真的被挡着，所以子智能体的 `PermissionRequest` 被吞掉时，行写着 Running 或 `N subagents`，而产品其实停在对话框上。要让它进来就得把那两格按 agent 分开，单独排期。
...
| 参数 | 值 | 含义 |
| --- | --- | --- |
| `freshness` | `30s` | 距**上一次尝试**多久之后重新读取——只在列表**非空**、或上一次尝试**没有拿到答案**时 |
...
- **列表非空**按 `freshness` 重读。这一半不能省：一行可能需要**退休**，而其中一条路径不产生任何边沿——被 `SIGKILL` 的会话来不及删掉自己的记录，只有那条命令自己的 `pid` + `procStart` 校验看得出它是幽灵（见下）。
...
3. **请求状态**：分别验证 Input、两种审批形态（专用审批工具与 Bash 等普通工具）的 Approval 出现/解决及 Running 恢复；批准与拒绝两条路径都要覆盖，拒绝后既要验证 Turn 继续调用其他工具时恢复 Running，也要验证不再调用工具时由 `Stop` 收敛为 Completed；验证孤立 `PermissionRequest`、指名其他 tool 的 `PermissionRequest` 都不会误报 Approval；**自动审查（`approvalsReviewer = auto_review`）下两种审批形态都不得出现 Approval needed，而同一条 thread 的 `request_user_input` 仍必须出现 Input needed**。
