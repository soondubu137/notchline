...
| **Codex CLI — 普通本地交互式终端** | **L5 — 请求阅读** | 生命周期、目录/标题、当前 Turn 进展、审批/输入等待及请求阅读 | 仅默认 home 和经验证的本地 TUI。**所有请求均为只读，须在其自己的终端中回答**：Codex 会等待 Hook，而不是在旁边另外提问，因此保持连接会把 TUI 自己的提问顶掉（见第 3 节）。不支持自动已读退役和精确终端导航，原生验证边界见 §5.1 |
...
| 请求阅读与回答 | Desktop：普通审批可回答，`request_permissions` 和同步问题只读，异步问题仅预览。CLI：所有表单只读，须在其自己的终端中回答（见 §5.1） | 范围见第 3 节 | 两者均不支持 | 命令和问题集只读，保留选项、自定义文本限制及可选附加信息；不支持回答 |
...
| 已读退役 | 条件支持，依据该 Thread 自己的控制终端：Turn 结束之后的击键、粘贴或焦点上报，且该终端所属应用位于前台。Codex 设置 `ESC[?1004h` 与 `ESC[?2004h`，不开启鼠标追踪，因此与 Claude Code 不同，指针掠过窗口永远不算操作；TUI 自身的输出与 Turn 结束也不会移动访问时间。**回到窗口是否足够取决于终端应用**：2026-09-15 实测，Ghostty 会下发焦点进出上报，回到窗口即退役该行；Apple Terminal 两者都不下发，只能等待击键。上报只到达获得焦点的那个终端面：同一次切换中，另一窗口里的另一个 TUI 毫无变化。Desktop 同时运行的 Thread，以及没有应用祖先的 `tmux`/`screen`/`ssh`，不进入该判定，仅在手动移除、下一次提交或失去执行归属时离开。Desktop 未读集合的缺席对终端没有权威 |
