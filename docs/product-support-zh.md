...
Codex CLI 覆盖范围于 **2026-09-22** 对照当前实现、测试及保留的验收记录核对；其他产品声明保留 2026-09-15 的核对基线。此次文档核对不代表重新实测原生产品；CLI 原生基线仍为 **0.154.0**。以下等级仅适用于声明的范围；第 3 节逐项列出请求形态，第 5 节列出独立能力及使用条件。
...
| **Codex CLI — 普通本地交互式终端** | **L5 — 请求阅读** | 生命周期、目录/标题、当前 Turn 进展、审批/输入等待及请求阅读 | 仅默认 home 和经验证的本地 TUI。**所有请求均为只读，须在其自己的终端中回答**：Codex 会等待 Hook，而不是在旁边另外提问，因此保持连接会把 TUI 自己的提问顶掉（见第 3 节）。已读移除需要在该 Thread 自己的终端上操作；不支持精确终端导航，原生验证边界见 §5.1 |
...
| 产品及原生形态 | 识别与阅读 | 从刘海回答 |
| --- | --- | --- |
| Codex Desktop：普通 `PermissionRequest` | 审批等待及所提供的请求字段 | 在 hook 连接保持有效期间批准或拒绝；拒绝时可附带文本 |
| Codex CLI：普通 `PermissionRequest` | 审批等待及所提供的请求字段 | 不支持：其连接会立即释放，交由 TUI 向本人提问，须在 Codex 中回答 |
...
| 已读后移除 | Desktop：其按 Thread 提供的未读状态。CLI：在该 Thread 自己的控制终端上的操作（见 §5.1）；两端的证据互不通用，两端同时运行的 Thread 任一端都不退役 | Desktop 记录和满足条件的前台证据；直接宿主终端中的操作 | Desktop：其按会话记录的查看时间，条件支持——用户离开该会话、在该会话上让 Desktop 窗口重新获得焦点或开始另一个会话时写入；一直看着 Turn 结束不会写入，直到上述操作发生。CLI：条件支持——Turn 结束后，在对应终端应用位于前台时键入或粘贴；仅返回标签页或移动指针不会清除监测行 | 有条件支持 IDE 与 IDE 内 SOLO，包括多个主窗口：Trae 位于前台、屏幕可用，且新鲜的原生窗口焦点证据与可见完成区域匹配；仅导航成功不够 |
...
普通本地交互式 CLI 是现有 Codex 产品的第二个端，共用一个设置开关和账户读取器。其等级为 **L5**：累积覆盖生命周期与计时（L1）、目录/标题（L2）、当前 Turn 进展（L3）、普通审批与同步问题的等待（L4），以及这些请求的内容（L5）。**所有 CLI 请求均不支持 L6 回答**。文档其他部分的 Desktop 覆盖范围不能在缺少独立证据时扩展到 CLI 变体。原生验收使用 **0.154.0**、临时 home、伪终端及确定性的本机模型服务；未使用用户凭据，也未修改用户配置。
...
| 普通命令审批 | 仅检测等待并阅读请求，决定在 TUI 中作出。曾通过生产 listener、Provider 和保持的 Hook 回答通道验证批准与拒绝（批准执行了无副作用的 printf 并产生 PostToolUse；拒绝未执行命令，随后出现 Stop），但**已不再提供**：那次验收没有检查 TUI 是否同时也在提问，而它并不提问。Codex 会等待 Hook，因此只要 Notchline 处于运行状态，终端就一直显示 `Working`，问题只出现在 notch 上；退出 Notchline 后终端立刻恢复提问（2026-09-15 反馈）。现在读取完载荷即释放连接，把提问留在用户正坐着的终端里。等待的解除方式不变——始终来自原生证据，而不是来自回答 |
| 同步问题 | 检测等待并阅读问题集，须在 Codex 中回答。原生 CLI 表单在预检阶段验证；精确取消与等待清理由集成测试覆盖 |
| 其他请求与子代理 | 尚未独立验收 CLI 覆盖：异步问题、`request_permissions`、自动审查模式、子代理计数/活动及子代理请求变体。共用投影可处理匹配的原生证据，但存在实现不等于 CLI 已验收，也不扩展已声明的 L5 请求形式 |
| 已读退役 | 条件支持，依据该 Thread 自己的控制终端：Turn 结束之后的击键、粘贴或焦点上报，且该终端所属应用位于前台、屏幕可用。操作识别的是 TUI，而非当前显示的 Thread：`/new` 后，它可移除该 TUI 仍持有的所有已结束 Turn。Codex 设置 `ESC[?1004h` 与 `ESC[?2004h`，不开启鼠标追踪，因此与 Claude Code 不同，指针掠过窗口永远不算操作；TUI 自身的输出与 Turn 结束也不会移动访问时间。**回到窗口是否足够取决于终端应用**：2026-09-15 实测，Ghostty 会下发焦点进出上报，回到窗口即退役该行；Apple Terminal 两者都不下发，只能等待击键。上报只到达获得焦点的那个终端面：同一次切换中，另一窗口里的另一个 TUI 毫无变化。Desktop 同时运行的 Thread 不进入该判定，仅在手动移除、下一次提交或失去执行归属时离开。SSH/终端复用器不属于 CLI 准入范围；即使收到不可到达的终端，已读判定也会拒绝。Desktop 未读集合的缺席对终端没有权威 |
| 导航 | 仅返回经验证的宿主，取决于宿主是否可达。由于无法确认当前显示的 Thread，禁用 TTY 选择；绝不启动 `resume` 或另一 CLI。导航结果不算已读证据；独立验证过的终端操作见上行 |
| 配额与 token | 共用默认账户的剩余配额、窗口时长/重置时间及今日 token 读数；当前仅展示一个配额窗口。不重复计数，也不单独归因 CLI 用量。API key/自定义服务的工作不证明消耗了 ChatGPT 配额；无凭据原生探针未验证登录后的配额行为 |
| 手动移除与结束原因 | 任何 Turn 状态均可手动移除，不停止工作。可观察精确 Interrupt；Hooks 不提供通用失败原因。Completed 不保证成功 |
| 调用范围 | 仅默认 `~/.codex`。接受已识别的本地交互式 `codex`、`resume` 和 `fork` 调用，但不恢复启动前 Turn。未知选项和有歧义的裸词调用保守拒绝 |
...
CLI 实现验证（2026-09-15）：使用 `-parallel-testing-enabled NO`，完整单元测试 **1,013 项通过**，零失败、零跳过；当日稍后修正审批支持范围（见 §5.1）后为 **1,029 项**。较早的并行运行中，四个未修改的选项/面板确认激活测试曾超时；完整串行运行全部通过。原生生命周期、进展及审批往返使用实际生产 listener 和 Provider，连接独立 CLI 与本机模型；其中审批往返记录的是该通道的能力，而不是支持范围——CLI 的请求为只读（见 §5.1）。默认 home 下的 Desktop 进程检查确认了 Desktop 自己的 server，并排除了 Notchline 的独立 server。
