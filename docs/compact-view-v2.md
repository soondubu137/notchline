...
> **Unchanged in substance, and its reach is now the whole app** ([`colour-v2.md`](colour-v2.md)). This section said hue stops meaning *which product* **on the collapsed bar**; the decision says it on every surface, and hands the same ink to the badge that names a product on a row and in the footer. `Mark colour` is `Theme colour` for that reason. Nothing about the palette, the default or the values moves.
...
- **The `8` is what is laid out; `12` is what is drawn**, and the two are not the same number because the wing's reading never leaves the `.clear` ground it keeps for the width it bills (§4.2) — so the ground's own `4` of padding stands between the mark and the first digit. Stated because the panel got it wrong: a row's reading has no ground on most states, spent the `8` alone, and stood the same mark `4` nearer the same digits than the bar did. A row with no ground pays that padding itself now (`PanelMetrics.drawnFinishDotGap`), and the row's digits do not move for it — the slot is trailing-anchored, so what the `4` moves is the dot (2026-09-09).
...
2. **`Hide the wings` (§9).**
3. ~~**The first-run anatomy page goes stale, knowingly.**~~ **Redrawn on 2026-09-06** ([`figma-design.md`](figma-design.md) §7.1). `OnboardingAnatomy.swift` pinned six labels on the V1 bar — `Codex`, `Claude Code`, `One dot per session`, `Most urgent state`, `Subagents`, `Longest turn` — and four of the six named things V2 removed. The key is now `Most urgent state`, `Sessions on the list`, `Subagents`, `The Project`, `A finished turn, unread`, `Longest turn`: the two that were still true, and four naming what V2 put there instead.

   **What the staleness cost while it stood was more than four wrong words.** Two of the pins were computed from the same x — the dot column's and the badges' — and once both parts were gone they evaluated to one point and were drawn on top of each other, which nothing failed for. A pin follows the product only in *where* it lands; its name does not follow anything. `theFirstRunPinsStandApartAndTheirLeadersStopAtTheEdge` now fails on the collision, and the key is spelled out in `figma-design.md` §7.1 so a name that stops being true has somewhere to be caught.
...
| The aggregate mark's ink | **Done**, as `Mark colour` — **renamed `Theme colour` by [`colour-v2.md`](colour-v2.md) §6**, because the same pair now tints every product badge as well as the mark. Twelve hues at `hint` (`AggregateInk`), ordered farthest-from-both-products first and nothing blocked. *(That ordering outlives its own premise: with no product hues left there is nothing to be far from, and the list keeps the order because re-sorting a shipped picker moves every row for no gain.)* The list is twelve plain names: the `★` on the two equidistant hues and the lit-colour dot on every row are both gone, replaced by a reference mark beside the popup that runs the radar twice round in the chosen ink. Default Sage; existing installs take the same, because until the row existed the mark was drawn at exactly it |
