...
## First native companion consumer

Trae local IDE now composes the shared lifecycle, row-content and request-reading contracts without Hooks or a second reducer. Its setup uses `ProductSetup.companionExtension`; the existing no-setup and managed-Hooks cases retain their boundaries. The [Trae integration record](trae-integration.md) contains the native L5 scope, private dependency registration, conformance fixtures and Release measurements. This integration is an implemented consumer of the five packages, not an increase to the other products’ support levels.
