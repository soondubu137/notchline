...
**Unread terminal state** — The Turn has reached a terminal state but its Thread has not been looked at. **Evidence is sourced per product**, and a product that supplies none keeps a finished row until its Thread's next submission, the Thread going away, or a right-click. Codex Desktop uses its unread set (the blue dot). Local Codex CLI supplies no read evidence: `/new` emits its new SessionStart only on the next submission, so a Hook-owned execution cannot prove the currently displayed Thread. CLI rows therefore stay until manual dismissal, the next submission or loss of execution ownership. Desktop unread absence never retires a CLI-owned row. Claude Code uses any one of five paths, which are one sentence in five shapes — *while the answer was in front of the user, the user did something only a person does*:
