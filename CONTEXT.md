...
**派生状态**：
「这条 Thread 还在不在干活」的答案，由**会话状态**加随行数据算出，不是第五种状态。只有一处会与会话状态分叉：Codex 一条 Thread 自己的处理轮次已经 Completed，而它派生的子智能体还在跑——那时会话状态是 Completed（轮次的确结束了），派生状态是 Running（这条 Thread 的确还在工作）。读它的只有那些本来就在问后一个问题的规则：汇总状态、产品标记、列表排序、终态未读成员关系门。**行自己画什么绝不读它**——行报告的是它那一个轮次。
