...
**汇总状态**：
由所有实时监视会话归并得到、显示在顶部的单一状态。跨产品归并：任一产品有会话时按会话优先级取值，只有所有产品都没有会话时才轮到集成可用性说话。
_Avoid_: 全局会话、连接状态

**断开（Disconnected）**：
某个产品的实时监视集成不再可靠，因而不能可信地产生该产品的会话集合。它按产品成立：一个产品断开时，它的矩阵与行一起消失——与"该产品没有会话"同形，因为对用户来说结论相同；只有所有产品都断开时才会成为汇总状态。
_Avoid_: 单会话未知、额度不可用、预览不可用、全局断开
