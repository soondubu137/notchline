...
Notchline summarises, with minimal interruption, the Turns a user still needs to attend to, and offers a way back into the originating Thread. The monitored products are Codex (Desktop and local interactive CLI), Claude Code, Antigravity (Desktop and CLI), and Trae Desktop (local IDE).
...
**Product** — A monitored agent product; today Codex, Claude Code, Antigravity at L3, and Trae at L5 with declared mode and request-form limits ([`docs/product-support.md`](docs/product-support.md) §5). A product is user-visible: row attribution and the footer's quota rules are per product. **A hue is not** — no product owns a colour anywhere in this app, and a product is named by a badge in the user's own theme ink ([`docs/colour-v2.md`](docs/colour-v2.md)). *Matrix hue* was on this list until that decision and is banned wording now, along with *product colour* and *product ink*.
...
**Navigable root Thread** — A Thread that belongs to no other Thread, whose identity the product confirms, and for which the declared return route is available. Codex Desktop can open the exact Thread; local Codex CLI returns only to its verified host, without claiming the TUI displays that Thread ([support scope](docs/product-support.md#51-local-codex-cli)). **The product answers this itself, and only "yes" counts**: the product hands over the Thread and it passes the check. Never asking, or an answer of "no such thread", is not a yes ([ADR 0017](docs/adr/0017-a-row-requires-a-thread-the-app-server-vouches-for.md)).
*Avoid:* related thread, all local threads, subagent thread.

**Side chat** — A temporary branch opened inside a Thread, listed only in its parent's UI and gone when the app closes. **Codex and Claude Code each have one; neither can become a row here.** Codex's is an ephemeral thread: it has its own thread id and fires Turn hooks as usual, but is never persisted, never listed, refused by `thread/read`, and has no deep link; its link to the parent exists only in Desktop's process memory. So it is not a navigable root Thread and cannot be filed under its parent (ADR 0017). Claude Code's is a read-only fork opened by Claude Desktop (`settingSources: []`, `persistSession: false`) that never reaches this app's observation surface at all — no hooks, absent from `claude agents --json`, no transcript. **That one is not stopped by a rule; it simply never speaks**, so no check is needed and none is written.
*Avoid:* sub-thread, temporary thread, subagent.

Trae’s native `side_chat` value names a persistent local IDE root Thread. It is not this glossary’s *Side chat*: its own `getSession` must confirm the same ID, root ownership and local scope before admission.
...
**Unread terminal state** — The Turn has reached a terminal state but its Thread has not been looked at. **Evidence is sourced per product**, and a product that supplies none keeps a finished row until its Thread's next submission, the Thread going away, or a right-click. Codex Desktop uses its unread set (the blue dot). **Local Codex CLI uses path 5 below and nothing else**, since Desktop keeps no record of a Thread it never ran: the TUI that owns the Thread answers through its controlling terminal. The gesture is narrower than Claude Code's, as Antigravity's is — measured 2026-09-15 on `0.154.0`, the TUI enables focus reporting and bracketed paste but no mouse reporting, so a keystroke, a paste and coming back to that window stamp the access time and the pointer cannot. **The last of those is the terminal's to give**: measured the same day, Ghostty delivers focus in and out to the focused surface alone — a TUI in another of its windows was untouched across the same switch — while Apple Terminal delivers neither, so there the row waits for a keystroke. Execution ownership is still not a current-view signal and this does not make it one: `/new` announces its Thread only on the next submission, so a gesture retires **every ended Turn that TUI owns**, which is a claim about the person being at that terminal — the same claim every other terminal row is retired on — not about which Thread was drawn. **Neither surface's evidence crosses to the other**: Desktop unread absence never retires a CLI-owned row, a terminal never answers for a Desktop window, and a Thread both surfaces run is retired by neither and stays until manual dismissal, the next submission or loss of execution ownership; `tmux`, `screen` and `ssh` executions are excluded from the supported CLI scope, and the read gate also refuses an unreachable terminal if supplied one. Claude Code uses any one of five paths, which are one sentence in five shapes — *while the answer was in front of the user, the user did something only a person does*:
...
**Trae local IDE and IDE-hosted SOLO** use fresh per-window evidence: the native focused main window displays the matching ended Turn and its completion control is visible, while Trae holds the macOS foreground and the screen is available. Multiple main windows are supported; neither another window's focus nor navigation alone means read. Unavailable evidence retains the row.

*Avoid:* recently completed, brief terminal state, notch unread.

## Status

**Thread status** — The product status of a live monitored Thread's current Turn. Four values only: Approval needed, Input needed, Running, Completed — which is also the summary and sort priority order ([`docs/PRD.md`](docs/PRD.md) §6.2). A missing, timed-out or unknown signal creates no fifth status; the last trustworthy value is held. The four are one vocabulary shared by all monitored products: a status one product can observe and the other cannot would make that vocabulary lie on the other (see *Terminal reason*).
...
**Presence** — Whether a product is open right now, independent of whether this app can observe it. Sourced per product: Codex by whether the app is running, Claude Code by whether the active-session list is non-empty — with no app to ask, the session list is the presence signal — and Antigravity by whether Antigravity Desktop is running or an `agy` process holds a conversation's presence lock open, read off the kernel. Trae presence comes from its running macOS application; the companion’s health separately decides whether it is observable. **Presence answers only "is it open", never "what is it doing"**: presence draws the matrix and the Turn reducer lights it, and the two must not be recombined. It has three values — open, not open, and unknown; unknown is what it takes when the only source of an answer has been failing past the trustworthy limit, and like "not open" it does not amount to Connected.
*Avoid:* availability, connection state, whether any thread is running.

**Connected** — A product that is both **open** and **observable**. These are independent facts and may contradict each other — open but with hooks unregistered is an ordinary first run — so only both together count as Connected. Where setup activation still requires verification or a known window reload, the product cannot yet claim Connected; Products and the summary use the same snapshot predicate.
*Avoid:* open, integration available, Ready.

**Monitoring intent** — The user's choice to enable or disable a product's observation in Notchline. Stored independently of actual installation, setup integrity, presence and observation health. Missing settings do not change enabled intent and never authorise automatic repair. A legacy complete setup can be adopted once; absence without a recorded choice means not set up. The Products switch expresses this intent ([`docs/product-connections.md`](docs/product-connections.md)).
...
**Project** — The grouping a Thread belongs to, resolved per product. In Codex Desktop it is the Thread grouping the user creates and manages in Codex Desktop, which may span one or more repositories; **a Codex Desktop Thread's Project must never be inferred from `cwd`, the Git root or a path name**, as those do not map one-to-one onto a user-managed Desktop Project ([ADR 0003](docs/adr/0003-use-codex-desktop-project-identity.md)). In local Codex CLI it is the native `cwd` grouping, with the shared missing-name fallback; this is not a Desktop Project. A Thread owned on both Codex surfaces retains Desktop Project semantics. In Antigravity Desktop it is the Project Desktop files the conversation under, read from Desktop's own records and never from the folder, or `Standalone` when it is filed under none; in Antigravity CLI, which has no Project object, it is the workspace path the product supplies, with an explicit missing-name fallback when absent. In Claude Code it is simply the Thread's working directory (`cwd`): the hook payload states it directly and transcripts are filed by it, so it is a grouping that genuinely exists in that product rather than an approximation read off a path ([ADR 0009](docs/adr/0009-resolve-project-per-product.md)).
...
**Request** — What one agent is asking a person, in the product's own words: a command to grant, a document to accept, a question with options, or a question with none ([`docs/answer-in-notch.md`](docs/answer-in-notch.md) §2.1). It is a thing the *product* composed and this app only draws — the app never annotates it, never marks a command as dangerous, and never summarises it. One agent may have several open at once; each is held under its own identity (the product's, or the call it concerns), resolving one never clears another, and the row opens one at a time, selected deterministically and pinned while a person reads it or until they explicitly select another. Browsing a request or a question never submits an answer. The shared identity includes the observation epoch, Thread, producer, native Turn and request identities, any native revision, and a fresh occurrence for a reopened or changed request.
...
### Trae’s observation boundary

Trae’s Project is the local workspace folder reported by its native session or Project record, named by its final path component; a confirmed absent folder is `Untitled folder`. Its Thread title is the displayed native name, with `Untitled` for an empty name. Read evidence is supplied per window (see *Unread terminal state*). A finished Trae row remains until it is read, the next observed Turn, manual dismissal or the application’s monitoring lifetime ending. Reconnecting can correct a Turn observed in the same lifetime; the initial baseline cannot admit an already-running or historical Turn. A broken companion hides its rows without declaring those Turns completed.
