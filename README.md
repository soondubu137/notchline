...
- Side chats are not monitored, in either product. A side chat is a temporary aside inside its parent conversation. Codex keeps its own entirely within the running Desktop window: it fires Turn hooks like any other thread, but nothing outside identifies it, ties it to the conversation it belongs to, or opens it — so Notchline draws no row for one, and more generally draws no row for any Turn whose thread Codex will not hand over. A Claude Code side chat never reaches Notchline in the first place: Claude Desktop runs it as a read-only fork that loads no settings, so it fires no hook, joins no session list, and leaves no transcript.
