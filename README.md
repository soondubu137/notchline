...
No product recovers pre-launch Turn state. Read removal only clears a monitoring row; it never archives the Thread. Unsupported features and temporary read failures are different. See the [support contract and full capability matrix](docs/product-support.md) ([简体中文](docs/product-support-zh.md)), checked on 2026-09-13, for each level's requirements, exclusions, request forms and host/mode conditions. IDE-hosted Trae SOLO has partial native verification but no separate declared level; it is not included in the local IDE's L5 promise.
...
- **Trae** — Requires the verified 3.5.91 build in `/Applications/Trae.app`. L5 covers local IDE/V2 root Threads. IDE-hosted SOLO can expose ordinary lifecycle and structured questions, but the tested Turns supplied no preview text and manual command approval remains unverified; it stays outside the L5 declaration. Standalone SoloLite, remote workspaces and Plan/Spec are excluded; rich permission forms are outside request coverage. Start a new Turn after the companion connects. Requests are answered in Trae; navigating there does not clear a completed row. See [Trae integration](docs/trae-integration.md) and the [SOLO boundary tests](docs/technical-explorations/multi-product-provider-architecture/trae-solo-boundaries.md).
