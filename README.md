...
| **Codex CLI (local interactive)** | **L6**: ordinary command approvals | Native folder/title, current-Turn progress, final-answer preview and shared account readings | Default home only; host return without exact Thread selection; no automatic read removal; synchronous questions reading-only; remote/daemon/exec excluded ([scope](docs/product-support.md#51-local-codex-cli)) |
