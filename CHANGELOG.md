...
- **A Trae row now shows a prompt, live progress and a final answer, not just an occasional plan thought.** The companion previously read only a plan item's `thought`, which stays empty on most ordinary Turns and never carried the model's actual answer. It now also reads a step's own finish summary, the root message's streamed answer text, and — while nothing has streamed yet — the exact prompt that started the Turn, still never its reasoning. The prompt read initially assumed native text was a plain string; it is typed blocks, and never matched a real message. Requires companion 1.2.1 and reopened Trae windows. (`docs/trae-integration.md`.)
...
**Notchline watches a third product, Antigravity, and every product now plugs into one shared monitoring core.** Antigravity Desktop and CLI arrive through one switch as a folder and a registry entry, not a rewrite. Support is described as six levels, L1 to L6, with Antigravity at L3. A row that has several approvals or questions open now holds all of them, and says what really happened to an answer you sent. (`docs/product-support.md`.)

### Added

- **Antigravity, Desktop and CLI, through one switch.** Settings writes two lifecycle hooks into `~/.gemini/config/hooks.json`. A Turn then shows `Working...` from its first model call to its end. CLI rows are named by workspace folder and Desktop rows by Desktop's Project. Approvals and questions are not detected, and the Settings row says so under its switch. (`docs/technical-explorations/multi-product-provider-architecture/antigravity-cli.md`, `antigravity-desktop.md`.)
- **An Antigravity row is titled and narrated from its transcript.** No hook payload carries the prompt or anything the model says. Both are read from the transcript the payload names. The title appears while the Turn still runs, the line under it updates after each model call, and a finished row shows the closing words.
- **An Antigravity row clears once it has been read.** A CLI row retires once you type in its terminal with that terminal in front, or when `agy` exits. A Desktop row retires once Desktop records the conversation as viewed. Clicking a Desktop row raises Desktop. Under `tmux`, `ssh` or a pipe, a CLI row keeps the previous lifecycle.
- **A row with several requests open lets you step through them.** A second approval on one agent used to overwrite the first. Every open request is now held. The row shows `Request 1 of N` with previous and next controls, keeps each request's draft and question position, and opens the next request once one is answered. (`docs/product-generalisation-plan.md`.)

### Changed

- **An answer's result says what delivery proved.** A failed send used to read "the product stopped waiting" whatever the cause. The row now tells apart an expired window, a request that can no longer be answered here, a product's own rejection, and a write that may or may not have landed. A connection whose answer window has run out is released, and its request stays readable.
- **Settings states each product's boundary under its switch.** Unless a failure takes the line, a product below L6 shows what it will never report, so a missing approval is stated up front instead of discovered later. The README and `docs/product-support.md` describe every product by level.
- **Antigravity presence is read with a fraction of the syscalls.** The process scan built and stat'ed a `URL` for every process on the machine. It now filters by kernel process name and compares paths as strings, which matters because a row waiting to be read runs this scan once a second.

### Fixed

- **A question option whose label contains a comma is answered as one choice.** Answers were a joined string, so `A, B` became two options, and two options with the same label became one. Answers now carry the chosen positions. A question drawn over a connection that cannot carry an answer no longer offers Submit.
- **An approval without an id is no longer filed under the wrong call.** When one tool ran twice in parallel, the approval borrowed the latest call. It now pairs with the one call it can only mean, or with none.
- **The quota footer stops drawing `-- left` for a product with no quota.** Antigravity gets the outer row alone. (`docs/quota-footer-v2.md` §5.)

### Known limitations

- **An Antigravity row's line lags behind a running tool.** Nothing fires between a sentence and the tool after it, so words written before a long command appear only once that command returns.
- **Reading an Antigravity CLI answer without typing keeps its row.** The CLI turns on no focus reporting. Asking the terminal which tab is showing was measured at about 0.6 s of CPU a minute and declined. (`antigravity-cli.md` §3.2.)
- **A Desktop Turn stopped with Stop execution keeps `Working...`** until the next Turn, Desktop quitting, or a right-click, because Desktop sends no end for it. A Desktop row cannot open its conversation; a click raises the app. (`antigravity-desktop.md` §3.)
...
- **`Privacy Mode` covers every word Notchline draws.** Settings, the Display card, second row. On, each row's project, title and latest line becomes a fixed-length bar and the collapsed pill stops naming the work; the mark, the counts, the clock, the product badges and the quota footer are untouched. Opening a row uncovers that row. Off by default, and remembered across launches. (`docs/cover-the-words.md`.)
- **A covered panel waits for a click, and a secondary press is the gesture.** The leak this exists for is a `0.15 s` dwell nobody intended: a pointer crossing the component on its way to a menu bar item puts a model's live answer on a shared screen. Covered, that crossing does nothing and a press on the band opens the panel instead. The band also turns the mode on and off, without opening Settings. (`docs/cover-the-words.md` §5 and §6.)
- **A covered row still shows it is working.** The searchlight that crosses a running row's last line crosses the bar that covers it, at the same period and in phase with every other row: covering takes the content away, not the reading. Its crest is the bar's own ink at full strength rather than white, so the brightest thing on a covered panel is still never a mark carrying no attention. (`docs/cover-the-words.md` §4.4.)
- **Hold the control in the band to read your own list.** Its glyph is the covered row itself — three stacked pills, short, medium, long. Every cover lifts while it is held, live and retired alike, and goes back on release; the mode stays on throughout, and the peek cannot be left on. VoiceOver's activation latches instead, because a held press has no keyboard equivalent. The control is drawn only while something is covered. (`docs/cover-the-words.md` §7.)
- **A panel opened by that press stays open, and leaving the mode under the pointer opens it.** Both follow from one rule: declining to expand on hover is not declining to hear the pointer. An entry still takes back a pending collapse whichever mode the surface is in, and the mode ending with the pointer on the component is offered the hover the tracking area cannot repeat. (`docs/cover-the-words.md` §6 and §11.2.)

### Fixed

- **A row can no longer be read through the heading that names its block.** A heading's line drew its `16` of black only while the list was taller than its viewport — and a list that fits still moves under one: the leading chip sits at the viewport's top at every offset, and a resize in flight leaves the scroller at its old height around content already at its new one. The ground travels with each heading now. (`docs/expanded-panel-v2.md` §4.6.)

### Known limitations

Everything listed under `0.2.5` and earlier still stands. `Privacy Mode` adds two of its own, both boundaries rather than gaps waiting to be filled:

- **It covers what is drawn, not what is stored.** The question it answers is what is on a screen somebody else is looking at. It says nothing about where message text lives or which socket it arrived on, and no clause of it may be cited against an engineering decision about storage or transport. (`docs/cover-the-words.md` §2.)
- **Neither collapsed form says the mode is on.** The notched bar has no free channel — brightness and size belong to the counts, hue to the user — and the pill declines one so that both forms keep a single rule. On a collapsed screen the state is legible only through the first pointer pass doing nothing, which is noticed in seconds. This is the design's own weakest point and is accepted rather than argued away. (`docs/cover-the-words.md` §8.)
