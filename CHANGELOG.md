...
- **A covered row still shows it is working.** The searchlight that crosses a running row's last line crosses the bar that covers it, at the same period and in phase with every other row — the channel that answers live or finished survives the cover, because covering takes the content away and not a reading. The crest is the bar's own ink at full strength rather than white, so the brightest thing on a covered panel is still never a mark that carries no attention. (`docs/cover-the-words.md` §4.4.)
- **Hold the control in the band to read your own list.** Its glyph is the covered row itself — three stacked pills, short then medium then long. While it is held every cover lifts at once, live rows and retired ones, and they go back on release. Privacy Mode stays on throughout — the peek is not a way out of it, and it cannot be left on: the panel closing and the mode ending both end it. VoiceOver's activation latches instead, because a held press has no keyboard equivalent. The control is drawn only while something is covered, and it stands to the left of the app's two existing controls so neither of them moves. (`docs/cover-the-words.md` §7.)
