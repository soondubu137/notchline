...
- **The live session list is grouped by product.** While both products are connected the list draws one block per product that has a row — Codex first, then Claude Code, in the fixed order the quota footer already keeps — instead of interleaving them by urgency alone. The block's heading is the Recent seam's own bar with the product's badge standing where the label stands and no chevron, so the rows under it give their chip up and carry the Project alone; VoiceOver keeps the product on every row. The chip sits on the bar's bottom edge rather than centred in it, so the heading stands `16` clear of the band's hairline and `12.5` from the caption it heads — a heading belongs to what it heads, and centring is right for a bar that closes a list rather than one that opens a block. A heading is never paid for out of rows: the viewport's cap grows by `32` for each one drawn, so three rows are still three rows and the panel is `64` taller at two products and not one point taller at one. The Recent queue is deliberately **not** grouped — a retired row's whole reading is an age, and today those ages run in one descent that a heading would restart at every block. What this gives up is stated rather than finessed: the first row on the panel is now the most urgent row of the first product rather than of the whole surface, and a block whose count is drawn lit is the panel saying somebody is waiting inside it. (`docs/expanded-panel-v2.md` §4.)

### Fixed

- **A Codex question no longer disappears under the sentence after it.** Reported from a second machine: Codex Desktop was holding a question — with two options, a free-text field, `Skip` and `Send` — while the row beside it described a query rewrite and said `Working...`, with nothing anywhere to say a person had been asked anything. The tool is `request_user_input_async`, Codex's second question tool, which asks the user and **does not stop**: it answers the model in 51 ms and the Turn carries on, which is what the CLI's own instructions tell the model to do, so the question was overwritten within seconds by the next thing the Turn said. It is no longer read as an ordinary tool call: the question's words are kept for the rest of that Turn and drawn as the row's preview, above the step the Turn has moved on to, and below the final answer once it ends — a Turn the user stopped, which has no final answer, shows the question rather than the prompt. The row deliberately still says `Working...` rather than `Input needed`: an answer to this question arrives as a *new* Turn, and a Skip, a snooze or Codex Desktop's own auto-resolution reach this app as nothing at all, so a wait opened here could never be honestly withdrawn. (`docs/PRD.md` §7, `docs/tech-design.md` §9.2, `docs/system-architecture.md` §3.)
